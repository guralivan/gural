...
if saved_projects:
        ...
        with st.form("edit_project_form"):
            ...
            with col_edit_btn1:
                if st.form_submit_button("💾 Сохранить изменения", type="primary"):
                    # Расчет обновленных дат
                    if edited_dev_days > 0:
                        edited_dev_end = edited_dev_start + timedelta(days=edited_dev_days)
                        edited_prod_end = edited_prod_start + timedelta(days=edited_prod_days)
                    else:
                        edited_dev_end = edited_dev_start
                        edited_prod_end = edited_prod_start + timedelta(days=edited_prod_days)
                    
                    edited_shipping_start = edited_prod_end
                    edited_shipping_end = edited_shipping_start + timedelta(days=edited_shipping_days)
                    edited_processing_start = edited_shipping_end
                    edited_processing_end = edited_processing_start + timedelta(days=edited_processing_days)
                    edited_wb_start = edited_processing_end
                    edited_wb_end = edited_wb_start + timedelta(days=edited_wb_days)
            ...
            with col_edit_btn3:
                if st.form_submit_button("🔄 Пересчитать даты"):
                    # Автоматический пересчет дат на основе новых параметров
                    if edited_dev_days > 0:
                        edited_dev_end = edited_dev_start + timedelta(days=edited_dev_days)
    ...
    for project in filtered_projects:
        ...
        with st.expander("📊 Детали проекта", expanded=False):
            ...
            st.plotly_chart(fig_timeline, use_container_width=True, key=f"timeline_{project['id']}_main")
            ...
            with col_btn1:
                if st.button("✏️ Редактировать", key=f"edit_{project['id']}"):
                    st.session_state.editing_project = project['id']
                    st.rerun()
                    
            with col_btn2:
                if st.button("🗑️ Удалить", key=f"delete_{project['id']}"):
            ...
            with col_btn3:
                if st.button("📊 Экспорт", key=f"export_{project['id']}"):
