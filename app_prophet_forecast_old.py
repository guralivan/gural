...
# Информация о Prophet
    ...
    st.markdown("### 💰 Прогнозирование расходов")
    ...
    st.markdown("### 📊 Прогнозирование полного отчета продаж")
    ...
    st.markdown("### 📋 Прогнозирование еженедельных отчетов")
    ...
    with st.spinner("Создаю прогноз..."):
        ...
        if prophet_df is not None and len(prophet_df) > 0:
            ...
            if model is not None and forecast is not None:
                ...
                # Отображаем график прогноза
                forecast_fig = plot_prophet_forecast(model, forecast, f"Прогноз: {metric_options[selected_metric]}")
                if forecast_fig:
                    st.plotly_chart(forecast_fig, use_container_width=True)
                
                # Отображаем компоненты
                components_fig = plot_prophet_components(model, forecast, f"Компоненты прогноза: {metric_options[selected_metric]}")
                if components_fig:
                    st.plotly_chart(components_fig, use_container_width=True)
                ...
                with col1:
                    st.metric(
                        ...
                        help="Среднее значение прогноза"
                    )
                ...
                with col2:
                    st.metric(
                        ...
                        help="Максимальное значение прогноза"
                    )
                ...
                with col3:
                    st.metric(
                        ...
                        help="Минимальное значение прогноза"
                    )
                
                with col4:
                    # Рассчитываем тренд
                    first_value = future_forecast['yhat'].iloc[0]
                    last_value = future_forecast['yhat'].iloc[-1]
                    trend = ((last_value - first_value) / first_value) * 100 if first_value != 0 else 0
                    
                    st.metric(
                        "Тренд (%)",
                        f"{trend:+.1f}%",
                        help="Изменение от начала к концу прогноза"
                    )
                ...
                display_forecast.columns = ['Дата', 'Прогноз', 'Нижняя граница', 'Верхняя граница']
                display_forecast['Дата'] = display_forecast['Дата'].dt.strftime('%d.%m.%Y')
                
                # Форматируем числовые значения
                for col in ['Прогноз', 'Нижняя граница', 'Верхняя граница']:
                    display_forecast[col] = display_forecast[col].round(0)
                
                st.dataframe(display_forecast, use_container_width=True)
                
                # Кнопка экспорта
                csv_data = display_forecast.to_csv(index=False, encoding='utf-8-sig')
                st.download_button(
                    label="📥 Скачать прогноз (CSV)",
                    data=csv_data,
                    file_name=f"prophet_forecast_{selected_metric}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                    mime="text/csv"
                )
...
**Источники данных:**
- **📈 Еженедельные отчеты (3.xlsx)** - расходы, логистика, штрафы
- **💰 Продажи (45.xlsx)** - заказы, выкупы, выручка
