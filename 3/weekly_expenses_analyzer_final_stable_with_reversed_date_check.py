    ...
    if has_investment and saved_amount > 0:
        ...
        if investments_list and len(investments_list) > 0:
            ...
            if start_date > first_investment_date:
                # Если дата выбрана после первого вложения, учитываем только вложения после выбранной даты
                st.info(f"ℹ️ Выбранная дата ({start_date.strftime('%d.%m.%Y')}) позже даты первого вложения ({first_investment_date.strftime('%d.%m.%Y')}). Учитываются только вложения после выбранной даты.")
    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                first_investment_date = min(inv['date'] for inv in investments_list)
                if start_date > first_investment_date:
                    # Если дата выбрана после первого вложения, показываем только вложения после выбранной даты
                    filtered_investments = [inv for inv in investments_list if inv['date'] >= start_date]
                    if filtered_investments:
                        filtered_saved_amount = sum(inv['amount'] for inv in filtered_investments)
                        st.metric(
