        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                if safe_date_compare(start_date, roi_calculation_date):
                    ...
                    st.metric(
                        label="💰 Итого вложено",
                        value=f"{saved_amount_from_second:,.0f} ₽",
                        delta=f"{len([inv for inv in investments_list if inv['date'] >= roi_calculation_date])} вложений (с {roi_calculation_date.strftime('%d.%m.%Y')})"
                    )
            else:
                # KPI Итого вложено
                st.metric(
            ...
            if safe_date_compare(start_date, roi_calculation_date):
                ...
                roi_selected_period = ((total_after_tax - saved_amount_from_second) / saved_amount_from_second) * 100 if saved_amount_from_second > 0 else 0
            ...
            if investments_list and len(investments_list) > 0:
                ...
                if safe_date_compare(start_date, roi_calculation_date):
                    ...
                    # Рассчитываем банковский доход (простой процент)
                    bank_income = saved_amount_from_second * (bank_interest_rate / 100) * (days_invested / 365)
                    bank_roi = (bank_income / saved_amount_from_second) * 100 if saved_amount_from_second > 0 else 0
            else:
                bank_income = 0
                bank_roi = 0
