    ...
    if has_investment and saved_amount > 0:
        ...
        if investments_list and len(investments_list) > 0:
            ...
            if start_date < first_investment_date:
                # Если дата выбрана до вложения, не учитываем вложение в расчетах
                st.warning(f"⚠️ Выбранная дата ({start_date.strftime('%d.%m.%Y')}) раньше даты первого вложения ({first_investment_date.strftime('%d.%m.%Y')}). Вложения не учитываются в расчетах.")
                roi = 0
    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                first_investment_date = min(inv['date'] for inv in investments_list)
                if start_date < first_investment_date:
                    # Если дата выбрана до вложения, показываем 0 вложений
                    st.metric(
                        label="💰 Итого вложено",
                        value="0 ₽",
                        delta="Вложения не учитываются (дата до вложения)"
                    )
                else:
                    # KPI Итого вложено
                    st.metric(
            ...
            if start_date < first_investment_date:
                # Если дата выбрана до вложения, ROI за выбранный период = 0
            ...
            if investments_list and len(investments_list) > 0:
                ...
                # Проверяем, не выбрана ли дата до вложения
                if start_date < first_investment_date:
                    # Если дата выбрана до вложения, банковский доход = 0
                    bank_income = 0
                    bank_roi = 0
                else:
            ...
            if investments_list and len(investments_list) > 0:
                # Проверяем, не выбрана ли дата до вложения
                if start_date < first_investment_date:
                    # Если дата выбрана до вложения, XIRR = 0
                    xirr = 0
                else:
