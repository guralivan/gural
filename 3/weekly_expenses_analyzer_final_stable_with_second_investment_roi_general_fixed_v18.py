...
def main():
    ...
    with tab1:
        ...
        with st.sidebar:
            ...
            recent_reports = get_recent_reports_for_entity('ЮЛ 1', max_count=3)
            if recent_reports:
                st.markdown("#### 🕒 Последние открытые отчеты:")
    ...
    with tab2:
        ...
        with st.sidebar:
            ...
            # Показываем последние открытые отчеты
            recent_reports_2 = get_recent_reports_for_entity('ЮЛ 2', max_count=3)
            if recent_reports_2:
                st.markdown("#### 🕒 Последние открытые отчеты:")
    ...
    with tab3:
        ...
        if all_investments:
            ...
            for inv in all_investments:
                # Проверяем, что вложение сделано после даты расчета ROI
                if safe_date_compare(inv['date'], roi_calculation_date_general) or inv['date'] == roi_calculation_date_general:
                    # Проверяем, что вложение попадает в выбранный период
            ...
            if use_first_investment_date or use_second_investment_date_general:
        ...
        with col3:
            st.metric(
                ...
                delta=f"{len(period_investments_general)} вложений (в выбранном периоде с {roi_calculation_date_general.strftime('%d.%m.%Y')})"
            )
