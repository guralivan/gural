    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                ...
                first_investment_datetime = pd.to_datetime(first_investment_date)
                days_invested = (end_date_datetime - first_investment_datetime).days
...
def main():
    ...
    with tab3:
        ...
        # Расчет ROI за выбранный период (не с даты первого вложения)
        roi_selected_period = ((total_after_tax - total_invested) / total_invested) * 100 if total_invested > 0 else 0
        ...
        if all_investments:
            ...
            # Рассчитываем количество дней с первого вложения до конца периода
            end_date_general = max_date.date()
            first_investment_date_obj = pd.to_datetime(first_investment_date).date()
            days_invested_general = (end_date_general - first_investment_date_obj).days
        ...
        # Разделитель между KPI и прогнозом ROI
        st.markdown("---")
        
        # Общий прогноз ROI с учетом остатков (отдельный раздел)
        st.markdown("## 🔮 Общий прогноз ROI с учетом остатков")
