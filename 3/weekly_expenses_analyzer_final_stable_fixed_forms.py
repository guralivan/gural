...
def analyze_single_file_data(df, file_name, tab_prefix=""):
    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            # Расчет ROI за весь период (не с даты первого вложения)
            roi_full_period = (total_after_tax / saved_amount) * 100 if saved_amount > 0 else 0
            ...
            st.metric(
                label="📊 ROI (весь период)",
                value=f"{roi_full_period:.1f}%",
                delta=f"За весь период"
            )
...
def main():
    ...
    with tab3:
        ...
        # Расчет ROI за весь период (не с даты первого вложения)
        roi_full_period = (total_after_tax / total_invested) * 100 if total_invested > 0 else 0
