...
def analyze_single_file_data(df, file_name, tab_prefix=""):
    ...
    if has_investment and saved_amount > 0:
        ...
        if investments_list and len(investments_list) > 0:
            ...
            if use_second_investment_date and len(investments_list) > 1:
                ...
                st.info(f"ℹ️ ROI рассчитывается с даты второго вложения: {roi_calculation_date.strftime('%d.%m.%Y')} (сумма вложений: {saved_amount_from_second:,.0f} ₽)")
            else:
    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                # Используем period_investments, которые уже рассчитаны выше
                if period_investments:
                    ...
                    st.metric(
                        ...
                        delta=f"{len(period_investments)} вложений (в выбранном периоде с {roi_calculation_date.strftime('%d.%m.%Y')})"
                    )
                else:
                    st.metric(
                        label="💰 Итого вложено",
                ...
                st.metric(
                    label="💰 Итого вложено",
                    value="0 ₽",
                    delta="Нет данных о вложениях"
                )
            
            # Эта логика уже обработана выше, удаляем дублирование
            pass
            ...
            for inv in investments_list:
                ...
                if inv['date'] >= roi_calculation_date:
