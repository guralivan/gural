...
def safe_date_compare(date1, date2):
    """Безопасное сравнение дат с автоматической конвертацией типов"""
    # Конвертируем date1 в datetime.date если это строка
    ...
    # Конвертируем date2 в datetime.date если это строка
...
def load_recent_reports_history():
    """Загружает историю последних открытых отчетов"""
    try:
        if os.path.exists('recent_reports.json'):
            with open('recent_reports.json', 'r', encoding='utf-8') as f:
                return json.load(f)
        return []
    except Exception as e:
        st.error(f"Ошибка при загрузке истории отчетов: {str(e)}")
        return []

def save_recent_reports_history(history):
    """Сохраняет историю последних открытых отчетов"""
    try:
        with open('recent_reports.json', 'w', encoding='utf-8') as f:
            json.dump(history, f, ensure_ascii=False, indent=2, default=str)
        return True
    except Exception as e:
        st.error(f"Ошибка при сохранении истории отчетов: {str(e)}")
        return False

def add_to_recent_reports(file_name, legal_entity, file_size=None, records_count=None):
    """Добавляет отчет в историю последних открытых"""
    history = load_recent_reports_history()
    
    # Создаем запись о новом отчете
    record_entry = {
        'file_name': file_name,
    ...
    # Удаляем дубликаты (если файл уже есть в истории)
    history = [r for r in history if not (r['file_name'] == file_name and r['legal_entity'] == legal_entity)]
    
    # Добавляем новый отчет в начало списка
    history.insert(0, record_entry)
    
    # Ограничиваем историю до 10 последних отчетов
    history = history[:10]
    
    # Сохраняем обновленную историю
    save_recent_reports_history(history)
    
    return history

def get_recent_reports_for_entity(legal_entity, max_count=5):
    """Получает последние отчеты для конкретного юридического лица"""
    history = load_recent_reports_history()
    entity_reports = [r for r in history if r['legal_entity'] == legal_entity]
    return entity_reports[:max_count]
...
def save_uploaded_report(uploaded_file, legal_entity):
    ...
    try:
        ...
        # Используем 'Дата формирования' для max_date, так как 'Дата конца' может содержать строки
        max_date = df['Дата формирования'].max()
        total_records = len(df)
        records_2024 = len(df[df['Дата формирования'].dt.year == 2024])
        records_2025 = len(df[df['Дата формирования'].dt.year == 2025])
        
        # Безопасная конвертация дат в строки
        def safe_date_to_isoformat(date_obj):
            """Безопасно конвертирует дату в ISO формат"""
        ...
        metadata = {
            ...
            'min_date': safe_date_to_isoformat(min_date),
            'max_date': safe_date_to_isoformat(max_date),
...
def calculate_monthly_expenses(df):
    """Рассчитывает расходы по месяцам"""
    if df.empty:
        return {}
    
    # Проверяем наличие столбца с датой
    date_column = None
    for col in ['Дата', 'Период', 'Неделя', 'Дата формирования']:
        if col in df.columns:
            date_column = col
            break
    ...
    df_copy['Месяц'] = df_copy[date_column].dt.to_period('M').astype(str)
    
    # Группируем по месяцам
    monthly_expenses = {}
    
    for month in df_copy['Месяц'].unique():
        month_data = df_copy[df_copy['Месяц'] == month]
        ...
        # Логистика
        if 'Стоимость логистики' in month_data.columns:
            expenses['logistics'] = month_data['Стоимость логистики'].sum()
        
        # Хранение
        if 'Стоимость хранения' in month_data.columns:
...
def analyze_single_file_data(df, file_name, tab_prefix=""):
        ...
        if has_investment and total_invested_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                ...
                if not df_from_investment.empty:
                    ...
                    total_after_tax_from_investment = expenses_from_investment.get('total_to_pay', {}).get('amount', 0) * 0.93  # минус 7% налог
    ...
    if has_investment and investments_list:
        ...
        with col2:
            ...
            if len(investments_list) > 1:
                ...
                if hasattr(second_investment_date, 'strftime'):
                    date_str = second_investment_date.strftime('%d.%m.%Y')
                else:
                    # Если это строка, пытаемся преобразовать
                    try:
                        date_obj = pd.to_datetime(second_investment_date)
                        date_str = date_obj.strftime('%d.%m.%Y')
                    except:
                        date_str = str(second_investment_date)
                
                st.checkbox(
                    f"Считать ROI с даты второго вложения ({date_str})",
    ...
    total_to_pay = expenses.get('total_to_pay', {'amount': 0, 'avg_per_week': 0})
    tax_amount = total_to_pay.get('amount', 0) * 0.07  # 7% налог
    total_after_tax = total_to_pay.get('amount', 0) - tax_amount
    
    # Общие суммы
    total_expenses = expenses['logistics']['amount'] + expenses['storage']['amount'] + expenses['other']['amount']
    total_amount = total_to_pay.get('amount', 0) + total_expenses
    ...
    if has_investment and saved_amount > 0:
        ...
        if investments_list and len(investments_list) > 0:
            ...
            if use_second_investment_date and len(investments_list) > 1:
                ...
                # Убеждаемся, что roi_calculation_date является объектом datetime.date
                if isinstance(roi_calculation_date, str):
                    roi_calculation_date = datetime.strptime(roi_calculation_date, '%Y-%m-%d').date()
                # Рассчитываем сумму вложений с даты второго вложения
                investments_from_second = [inv for inv in investments_list if inv['date'] >= roi_calculation_date]
                saved_amount_from_second = sum(inv['amount'] for inv in investments_from_second)
                roi_date_str = roi_calculation_date.strftime('%d.%m.%Y') if hasattr(roi_calculation_date, 'strftime') else str(roi_calculation_date)
                st.info(f"ℹ️ ROI рассчитывается с даты второго вложения: {roi_date_str} (сумма вложений: {saved_amount_from_second:,.0f} ₽)")
            else:
                roi_calculation_date = first_investment_date  # Первое вложение
                saved_amount_from_second = saved_amount  # Используем общую сумму
            
            # Проверяем, не выбрана ли дата после вложения
            if safe_date_compare(start_date, roi_calculation_date):
                ...
                roi_date_str = roi_calculation_date.strftime('%d.%m.%Y') if hasattr(roi_calculation_date, 'strftime') else str(roi_calculation_date)
                st.info(f"ℹ️ Выбранная дата ({start_date_str}) позже даты расчета ROI ({roi_date_str}). Учитываются только вложения после выбранной даты.")
                ...
                if not df_from_investment.empty:
                    ...
                    profit_after_tax_from_investment = total_after_tax_from_investment - saved_amount_from_second
                    roi = (profit_after_tax_from_investment / saved_amount_from_second) * 100 if saved_amount_from_second > 0 else 0
                    
                    # Прибыль после налога для отображения (с выбранного периода)
                    profit_after_tax = total_after_tax - saved_amount_from_second
                else:
                    # Если нет данных с даты вложения, используем текущий период
                    profit_after_tax = total_after_tax - saved_amount_from_second
                    roi = (profit_after_tax / saved_amount_from_second) * 100 if saved_amount_from_second > 0 else 0
        else:
            # Если нет списка вложений, используем текущий период
    ...
    with col1:
        st.metric(
            ...
            value=f"{total_to_pay.get('amount', 0):,.0f} ₽",
            delta=f"Среднее: {total_to_pay.get('avg_per_week', 0):,.0f} ₽/нед"
        )
    ...
    with col2:
        st.metric(
            ...
            delta=f"{(tax_amount/total_to_pay.get('amount', 0)*100):.1f}% от дохода"
        )
    ...
    with col3:
        # Расчет ROI за выбранный период с учетом вложений после даты первого/второго вложения
        # Фильтруем вложения: только те, которые сделаны после даты расчета ROI И в выбранном периоде
        period_investments = []
        if investments_list and len(investments_list) > 0:
            for inv in investments_list:
                # Проверяем, что вложение сделано после даты расчета ROI
                if safe_date_compare(inv['date'], roi_calculation_date) or inv['date'] == roi_calculation_date:
                    # Проверяем, что вложение попадает в выбранный период
                    # Вложение должно быть >= start_date И <= end_date
                    if (safe_date_compare(inv['date'], start_date) or inv['date'] == start_date) and (safe_date_compare(end_date, inv['date']) or inv['date'] == end_date):
                        period_investments.append(inv)
        
        # Метрики инвестиций (если есть вложения)
        if has_investment and saved_amount > 0:
            # Показываем вложения для выбранного периода
            if period_investments:
                period_invested_amount = sum(inv['amount'] for inv in period_investments)
                roi_date_str = roi_calculation_date.strftime('%d.%m.%Y') if hasattr(roi_calculation_date, 'strftime') else str(roi_calculation_date)
                st.metric(
                    label="💰 Итого вложено",
                    value=f"{period_invested_amount:,.0f} ₽",
                    delta=f"{len(period_investments)} вложений (в выбранном периоде с {roi_date_str})"
                )
            else:
                st.metric(
                    label="💰 Итого вложено",
                    value="0 ₽",
                    delta="Нет вложений в выбранном периоде"
                )
            
            if period_investments:
                roi_selected_period = ((total_after_tax - period_invested_amount) / period_invested_amount) * 100 if period_invested_amount > 0 else 0
            else:
                roi_selected_period = 0
            
            # Расчет банковского дохода для выбранного периода
            if period_investments:
                ...
                bank_income = period_invested_amount * (bank_interest_rate / 100) * (days_invested / 365)
                bank_roi = (bank_income / period_invested_amount) * 100 if period_invested_amount > 0 else 0
            else:
                bank_income = 0
                bank_roi = 0
            
            # Расчет XIRR для выбранного периода
            if period_investments:
                # Создаем денежные потоки для XIRR
                cashflows = []
                dates = []
                
                # Добавляем вложения из выбранного периода (отрицательные потоки)
    ...
    if tab_prefix:
        st.markdown(f"### 📋 Детальная таблица расходов по неделям - {file_name}")
    else:
    ...
    if 'Итого к оплате' in filtered_df.columns:
        # Создаем копию данных для анализа по годам - используем ВСЕ данные, а не только отфильтрованные
        year_comparison_df = df.copy()  # Используем полные данные вместо filtered_df
    ...
    <div class="total-card">
        ...
        <ul>
            ...
            <li><strong>Итого к оплате:</strong> {expenses.get('total_to_pay', {'amount': 0, 'avg_per_week': 0})['amount']:,.0f} ₽</li>
    ...
    if st.button("📥 Скачать отчет о расходах (Excel)", key=f"{tab_prefix}export_{legal_entity}"):
        ...
        # График расходов по месяцам для данного юридического лица
        st.markdown("### 📈 Расходы по месяцам")
        
        monthly_expenses = calculate_monthly_expenses(df_display)
        
        if monthly_expenses:
            # Создаем данные для графика
            months = list(monthly_expenses.keys())
            logistics = [monthly_expenses[month].get('logistics', 0) for month in months]
            storage = [monthly_expenses[month].get('storage', 0) for month in months]
            other = [monthly_expenses[month].get('other', 0) for month in months]
            total = [monthly_expenses[month].get('total', 0) for month in months]
            ...
            # Добавляем столбцы для каждой категории
            ...
            # Добавляем линию для общего итога
            ...
            # Настройка графика
            fig_monthly.update_layout(
                title=f'Расходы по месяцам - {legal_entity}',
                xaxis_title='Месяц',
                yaxis_title='Сумма (₽)',
                barmode='stack',
                height=500,
            ...
            # Создаем отдельный график для логистики и хранения
            fig_logistics_storage = go.Figure()
            
            # Добавляем линии для логистики и хранения
            fig_logistics_storage.add_trace(go.Scatter(
                x=months,
                y=logistics,
                mode='lines+markers',
                name='🚚 Логистика',
                line=dict(width=3, dash='solid', color='#1f77b4'),
                marker=dict(size=8)
            ))
            ...
            # Настройка графика
            fig_logistics_storage.update_layout(
                title=f'Расходы на логистику и хранение по месяцам - {legal_entity}',
                xaxis_title='Месяц',
                yaxis_title='Сумма (₽)',
                height=500,
                showlegend=True,
...
def main():
    ...
    with tab1:
        ...
        with st.sidebar:
            ...
            # Кнопка очистки истории последних отчетов
            if st.button("🗑️ Очистить историю отчетов", key="clear_recent_history_1"):
                if os.path.exists('recent_reports.json'):
                    os.remove('recent_reports.json')
                st.success("✅ История последних отчетов очищена")
                st.rerun()
            
            # Настройка автоматической загрузки
            st.markdown("#### ⚙️ Настройки автозагрузки:")
            auto_load_enabled = st.checkbox(
                "🔄 Автоматически загружать последний файл при запуске",
                value=True,
                key="auto_load_setting_1",
                help="При включении приложение автоматически загрузит последний открытый файл"
            )
            ...
            if legal_entity_1_reports:
                # Показываем только последний отчет в свернутом виде
                latest_report = legal_entity_1_reports[0]  # Первый в списке - самый новый
                upload_date = datetime.fromisoformat(latest_report['upload_date']).strftime('%d.%m.%Y %H:%M')
                
                with st.expander("📋 Сохраненные отчеты", expanded=False):
                    st.markdown(f"**{latest_report['original_name']}** ({upload_date})")
                    st.markdown(f"Записей: {latest_report['total_records']} (2024: {latest_report['records_2024']}, 2025: {latest_report['records_2025']})")
                    
                    # Кнопка для загрузки последнего отчета
                    if st.button(f"📂 Загрузить {latest_report['original_name']}", key=f"load_1_{latest_report['filename']}"):
                        df1 = load_saved_report(latest_report['filename'])
                        if df1 is not None:
                            df1 = load_expenses_data_from_df(df1)
                            st.session_state['df1'] = df1
                            st.session_state['file_name_1'] = latest_report['original_name']
                            st.success(f"✅ Загружен сохраненный отчет: {latest_report['original_name']}")
                            st.rerun()
            else:
                st.info("📋 Нет сохраненных отчетов для ЮЛ 1")
            
            # Показываем последние открытые отчеты
            recent_reports = get_recent_reports_for_entity('ЮЛ 1', max_count=2)
            if recent_reports:
                with st.expander("🕒 Последние открытые отчеты", expanded=False):
                    for report in recent_reports:
                        opened_date = datetime.fromisoformat(report['opened_at']).strftime('%d.%m.%Y %H:%M')
                        st.markdown(f"**{report['file_name']}** ({opened_date})")
                        if report.get('records_count'):
                            st.markdown(f"Записей: {report['records_count']}")
                        
                        # Кнопка для быстрого открытия
                        if st.button(f"⚡ Открыть {report['file_name']}", key=f"recent_1_{report['file_name']}"):
                            # Здесь можно добавить логику для быстрого открытия
                            st.info(f"🔄 Открытие отчета: {report['file_name']}")
                            # Добавляем в session_state для автоматической загрузки
                            st.session_state['auto_load_file_1'] = report['file_name']
                            st.rerun()
            else:
                st.info("🕒 Нет истории открытых отчетов")
        ...
        if uploaded_file_1 is not None:
            ...
            # Добавляем в историю последних открытых отчетов
            add_to_recent_reports(
                file_name=uploaded_file_1.name,
                legal_entity='ЮЛ 1',
                file_size=uploaded_file_1.size,
                records_count=len(df1) if df1 is not None else None
            )
            
            # Автоматически сохраняем загруженный отчет
            filepath, metadata = save_uploaded_report(uploaded_file_1, "ЮЛ 1")
            if filepath:
                st.success(f"✅ Файл {uploaded_file_1.name} успешно загружен и сохранен")
                st.info(f"💾 Отчет автоматически сохранен: {metadata['filename']}")
            else:
            ...
            # Кнопка для принудительного сохранения (если нужно)
            if st.button("💾 Принудительно сохранить отчет", key="force_save_report_1"):
                filepath, metadata = save_uploaded_report(uploaded_file_1, "ЮЛ 1")
                if filepath:
                    st.success(f"✅ Отчет принудительно сохранен: {metadata['filename']}")
                    st.rerun()
        elif 'df1' in st.session_state:
            # Используем сохраненный отчет из session_state
            df1 = st.session_state['df1']
            file_name_1 = st.session_state['file_name_1']
            st.info(f"🔍 Источник данных: Session state - {file_name_1}")
        elif 'auto_load_file_1' in st.session_state:
            # Автоматически загружаем последний файл из кеша
            auto_file_name = st.session_state['auto_load_file_1']
            st.info(f"🔄 Автоматическая загрузка последнего файла: {auto_file_name}")
            
            # Ищем файл в сохраненных отчетах
            saved_reports = load_report_metadata()
            matching_report = None
            for report in saved_reports:
                if report['original_name'] == auto_file_name and report['legal_entity'] == 'ЮЛ 1':
                    matching_report = report
                    break
            
            if matching_report:
                df1 = load_saved_report(matching_report['filename'])
                if df1 is not None:
                    df1 = load_expenses_data_from_df(df1)
                    st.session_state['df1'] = df1
                    st.session_state['file_name_1'] = matching_report['original_name']
                    st.success(f"✅ Автоматически загружен последний отчет: {matching_report['original_name']}")
                    # Очищаем флаг автоматической загрузки
                    del st.session_state['auto_load_file_1']
                    st.rerun()
                else:
                    st.error(f"❌ Ошибка при загрузке файла: {auto_file_name}")
                    del st.session_state['auto_load_file_1']
            else:
                st.warning(f"⚠️ Файл не найден в сохраненных отчетах: {auto_file_name}")
                del st.session_state['auto_load_file_1']
        else:
            # Пытаемся автоматически загрузить последний файл из кеша при первом запуске
            recent_reports = get_recent_reports_for_entity('ЮЛ 1', max_count=1)
            if recent_reports and 'auto_load_enabled' not in st.session_state and auto_load_enabled:
                last_report = recent_reports[0]
                st.info(f"🔄 Автоматическая загрузка последнего файла: {last_report['file_name']}")
                
                # Ищем файл в сохраненных отчетах
                saved_reports = load_report_metadata()
                matching_report = None
                for report in saved_reports:
                    if report['original_name'] == last_report['file_name'] and report['legal_entity'] == 'ЮЛ 1':
                        matching_report = report
                        break
                ...
                if matching_report:
                    ...
                    if df1 is not None:
                        ...
                        st.error(f"❌ Ошибка при загрузке файла: {last_report['file_name']}")
                        st.session_state['auto_load_enabled'] = True
                else:
                    st.warning(f"⚠️ Файл не найден в сохраненных отчетах: {last_report['file_name']}")
                    st.session_state['auto_load_enabled'] = True
            else:
    ...
    with tab2:
        ...
        with st.sidebar:
            ...
            # Кнопка очистки истории последних отчетов
            if st.button("🗑️ Очистить историю отчетов", key="clear_recent_history_2"):
                if os.path.exists('recent_reports.json'):
                    os.remove('recent_reports.json')
                st.success("✅ История последних отчетов очищена")
                st.rerun()
            ...
            # Настройка автоматической загрузки
            st.markdown("#### ⚙️ Настройки автозагрузки:")
            auto_load_enabled_2 = st.checkbox(
                "🔄 Автоматически загружать последний файл при запуске",
                value=True,
                key="auto_load_setting_2",
                help="При включении приложение автоматически загрузит последний открытый файл"
            )
            ...
            if legal_entity_2_reports:
                # Показываем только последний отчет в свернутом виде
                latest_report = legal_entity_2_reports[0]  # Первый в списке - самый новый
                upload_date = datetime.fromisoformat(latest_report['upload_date']).strftime('%d.%m.%Y %H:%M')
                ...
                with st.expander("📋 Сохраненные отчеты", expanded=False):
                    ...
                    # Кнопка для загрузки последнего отчета
                    if st.button(f"📂 Загрузить {latest_report['original_name']}", key=f"load_2_{latest_report['filename']}"):
                        df2 = load_saved_report(latest_report['filename'])
                        if df2 is not None:
                            df2 = load_expenses_data_from_df(df2)
                            st.session_state['df2'] = df2
                            st.session_state['file_name_2'] = latest_report['original_name']
                            st.success(f"✅ Загружен сохраненный отчет: {latest_report['original_name']}")
                            st.rerun()
            else:
            ...
            # Показываем последние открытые отчеты
            recent_reports_2 = get_recent_reports_for_entity('ЮЛ 2', max_count=2)
            if recent_reports_2:
                with st.expander("🕒 Последние открытые отчеты", expanded=False):
                    for report in recent_reports_2:
                        opened_date = datetime.fromisoformat(report['opened_at']).strftime('%d.%m.%Y %H:%M')
                        st.markdown(f"**{report['file_name']}** ({opened_date})")
                        if report.get('records_count'):
                            st.markdown(f"Записей: {report['records_count']}")
                        ...
                        # Кнопка для быстрого открытия
                        if st.button(f"⚡ Открыть {report['file_name']}", key=f"recent_2_{report['file_name']}"):
                            # Здесь можно добавить логику для быстрого открытия
                            st.info(f"🔄 Открытие отчета: {report['file_name']}")
                            # Добавляем в session_state для автоматической загрузки
                            st.session_state['auto_load_file_2'] = report['file_name']
                            st.rerun()
            else:
                st.info("🕒 Нет истории открытых отчетов")
        ...
        if uploaded_file_2 is not None:
            ...
            # Добавляем в историю последних открытых отчетов
            add_to_recent_reports(
                file_name=uploaded_file_2.name,
                legal_entity='ЮЛ 2',
                file_size=uploaded_file_2.size,
                records_count=len(df2) if df2 is not None else None
            )
            
            # Автоматически сохраняем загруженный отчет
            filepath, metadata = save_uploaded_report(uploaded_file_2, "ЮЛ 2")
            if filepath:
                st.success(f"✅ Файл {uploaded_file_2.name} успешно загружен и сохранен")
                st.info(f"💾 Отчет автоматически сохранен: {metadata['filename']}")
            else:
            ...
            # Кнопка для принудительного сохранения (если нужно)
            if st.button("💾 Принудительно сохранить отчет", key="force_save_report_2"):
                filepath, metadata = save_uploaded_report(uploaded_file_2, "ЮЛ 2")
                if filepath:
                    st.success(f"✅ Отчет принудительно сохранен: {metadata['filename']}")
                    st.rerun()
        ...
        elif 'auto_load_file_2' in st.session_state:
            # Автоматически загружаем последний файл из кеша
            auto_file_name = st.session_state['auto_load_file_2']
            st.info(f"🔄 Автоматическая загрузка последнего файла: {auto_file_name}")
            ...
            matching_report = None
            for report in saved_reports:
                if report['original_name'] == auto_file_name and report['legal_entity'] == 'ЮЛ 2':
                    matching_report = report
                    break
            
            if matching_report:
                df2 = load_saved_report(matching_report['filename'])
                if df2 is not None:
                    df2 = load_expenses_data_from_df(df2)
                    st.session_state['df2'] = df2
                    st.session_state['file_name_2'] = matching_report['original_name']
                    st.success(f"✅ Автоматически загружен последний отчет: {matching_report['original_name']}")
                    # Очищаем флаг автоматической загрузки
                    del st.session_state['auto_load_file_2']
                    st.rerun()
                else:
                    st.error(f"❌ Ошибка при загрузке файла: {auto_file_name}")
                    del st.session_state['auto_load_file_2']
            ...
            # Пытаемся автоматически загрузить последний файл из кеша при первом запуске
            recent_reports = get_recent_reports_for_entity('ЮЛ 2', max_count=1)
            if recent_reports and 'auto_load_enabled_2' not in st.session_state and auto_load_enabled_2:
                last_report = recent_reports[0]
                st.info(f"🔄 Автоматическая загрузка последнего файла: {last_report['file_name']}")
                ...
                matching_report = None
                for report in saved_reports:
                    if report['original_name'] == last_report['file_name'] and report['legal_entity'] == 'ЮЛ 2':
                        matching_report = report
                        break
                ...
                if matching_report:
                    ...
                    if df2 is not None:
                        ...
                        st.session_state['df2'] = df2
                        st.session_state['file_name_2'] = matching_report['original_name']
                        st.session_state['auto_load_enabled_2'] = True
                        st.success(f"✅ Автоматически загружен последний отчет: {matching_report['original_name']}")
                        st.rerun()
                    else:
                        st.error(f"❌ Ошибка при загрузке файла: {last_report['file_name']}")
    ...
    with tab3:
        ...
        # Галочки для расчета ROI в общем KPI
        if all_investments:
            ...
            with col2:
                ...
                if len(all_investments) > 1:
                    second_investment_date = all_investments[1]['date']
                    use_second_investment_date_general = st.checkbox(
                        f"Считать общий KPI с даты второго вложения ({second_investment_date.strftime('%d.%m.%Y')})",
                        value=st.session_state.get("general_kpi_second_date", False),
                        key="general_kpi_second_date",
                        help="При включении ROI будет рассчитываться с даты второго вложения"
                    )
                else:
                    st.info("ℹ️ Второе вложение отсутствует")
                    use_second_investment_date_general = False
            
            # Логика взаимоисключения галочек
            if use_second_investment_date_general and use_first_investment_date:
                # Если выбраны обе галочки, отключаем первую
                st.session_state.general_kpi_first_date = False
                use_first_investment_date = False
                st.warning("⚠️ Можно выбрать только одну галочку. Отключена галочка первого вложения.")
            
            # Показываем информацию о выбранной дате
            if use_second_investment_date_general and len(all_investments) > 1:
                st.info(f"📅 Общий KPI рассчитывается с даты второго вложения: {second_investment_date.strftime('%d.%m.%Y')}")
            elif use_first_investment_date:
        ...
        if use_second_investment_date_general and len(all_investments) > 1:
            # Используем дату второго вложения как начальную дату
            filter_start_date_general = second_investment_date
            st.info(f"📅 Фильтр автоматически установлен с даты второго вложения: {filter_start_date_general.strftime('%d.%m.%Y')}")
        elif use_first_investment_date and all_investments:
            # Используем дату первого вложения как начальную дату
        ...
        total_to_pay_general = expenses_general.get('total_to_pay', {'amount': 0, 'avg_per_week': 0})
        tax_amount_general = total_to_pay_general.get('amount', 0) * 0.07  # 7% налог
        total_after_tax_general = total_to_pay_general.get('amount', 0) - tax_amount_general
        
        # Общие суммы (для выбранного периода)
        total_expenses_general = expenses_general.get('logistics', {}).get('amount', 0) + expenses_general.get('storage', {}).get('amount', 0) + expenses_general.get('other', {}).get('amount', 0)
        total_amount_general = total_to_pay_general.get('amount', 0) + total_expenses_general
        
        # Процент расходов от общей суммы
        expenses_percentage_general = (total_expenses_general / total_amount_general) * 100 if total_amount_general > 0 else 0
        
        # Инициализируем переменные для ROI
        total_invested_from_second_general = 0
        period_investments_general = []
        roi_calculation_date_general = None
        
        # Расчет ROI и XIRR с учетом выбранной галочки
        if all_investments:
            # Определяем дату для расчета ROI
            if use_second_investment_date_general and len(all_investments) > 1:
                roi_calculation_date_general = all_investments[1]['date']  # Второе вложение
                # Убеждаемся, что roi_calculation_date_general является объектом datetime.date
                if isinstance(roi_calculation_date_general, str):
                    roi_calculation_date_general = datetime.strptime(roi_calculation_date_general, '%Y-%m-%d').date()
            else:
                roi_calculation_date_general = first_investment_date  # Первое вложение
            
            # Фильтруем вложения: только те, которые сделаны после даты расчета ROI И в выбранном периоде
            for inv in all_investments:
                # Проверяем, что вложение сделано после даты расчета ROI
                if roi_calculation_date_general and (safe_date_compare(inv['date'], roi_calculation_date_general) or inv['date'] == roi_calculation_date_general):
                    # Проверяем, что вложение попадает в выбранный период
                    # Вложение должно быть >= start_date_general И <= end_date_general
                    if (safe_date_compare(inv['date'], start_date_general) or inv['date'] == start_date_general) and (safe_date_compare(end_date_general, inv['date']) or inv['date'] == end_date_general):
                        period_investments_general.append(inv)
            
            # Рассчитываем сумму вложений в выбранном периоде
            total_invested_from_second_general = sum(inv['amount'] for inv in period_investments_general)
            
            if (use_first_investment_date or use_second_investment_date_general) and roi_calculation_date_general:
                # Фильтруем данные с даты расчета ROI (в рамках выбранного периода)
                df_from_investment = combined_df_filtered[combined_df_filtered['Дата формирования'] >= pd.to_datetime(roi_calculation_date_general)]
                
                if not df_from_investment.empty:
                    # Рассчитываем расходы с даты расчета ROI
                    expenses_from_investment = calculate_expenses(df_from_investment)
                    
                    # Обновляем данные для отображения с даты расчета ROI
                    ...
                    # ROI с даты расчета ROI
                    profit_after_tax_from_investment = total_after_tax - total_invested_from_second_general
                    roi = (profit_after_tax_from_investment / total_invested_from_second_general) * 100 if total_invested_from_second_general > 0 else 0
                    
                    # Прибыль после налога для отображения (с даты расчета ROI)
                    profit_after_tax = total_after_tax - total_invested_from_second_general
                else:
                    # Если нет данных с даты вложения, используем выбранный период
                    profit_after_tax = total_after_tax_general - total_invested_from_second_general
                    roi = (profit_after_tax / total_invested_from_second_general) * 100 if total_invested_from_second_general > 0 else 0
            else:
        ...
        roi_selected_period = ((total_after_tax_general - total_invested_from_second_general) / total_invested_from_second_general) * 100 if total_invested_from_second_general > 0 else 0
        
        # Расчет банковского дохода в общем KPI
        if all_investments and roi_calculation_date_general:
            # Рассчитываем количество дней с даты расчета ROI до конца выбранного периода
            days_invested_general = (end_date_general - roi_calculation_date_general).days
            
            # Рассчитываем банковский доход (простой процент)
            bank_income_general = total_invested_from_second_general * (bank_interest_rate_general / 100) * (days_invested_general / 365)
            bank_roi_general = (bank_income_general / total_invested_from_second_general) * 100 if total_invested_from_second_general > 0 else 0
        else:
        ...
        if all_investments:
            ...
            # Добавляем вложения из выбранного периода (отрицательные потоки)
            for inv in period_investments_general:
                cashflows.append(-inv['amount'])  # Отрицательные = вложения
                dates.append(inv['date'])
        ...
        with col1:
            st.metric(
                ...
                value=f"{total_to_pay_general.get('amount', 0):,.0f} ₽",
                delta=f"Среднее: {total_to_pay_general.get('avg_per_week', 0):,.0f} ₽/нед"
            )
        ...
        with col2:
            st.metric(
                ...
                delta=f"{(tax_amount_general/total_to_pay_general.get('amount', 0)*100):.1f}% от дохода"
            )
        ...
        with col3:
            st.metric(
                ...
                value=f"{total_invested_from_second_general:,.0f} ₽",
                delta=f"{len(period_investments_general)} вложений (в выбранном периоде с {roi_calculation_date_general.strftime('%d.%m.%Y') if roi_calculation_date_general else 'неизвестной даты'})"
            )
        ...
        with col4:
            st.metric(
                ...
                value=f"{expenses_general.get('logistics', {}).get('amount', 0):,.0f} ₽",
                delta=f"{(expenses_general.get('logistics', {}).get('amount', 0)/total_amount_general*100):.1f}% от общей суммы"
            )
            
            st.metric(
                label="📦 Сумма хранения (общий)",
                value=f"{expenses_general.get('storage', {}).get('amount', 0):,.0f} ₽",
                delta=f"{(expenses_general.get('storage', {}).get('amount', 0)/total_amount_general*100):.1f}% от общей суммы"
            )
        ...
        # График с метриками расходов
        st.markdown("### 📊 График расходов по категориям")
        ...
        # Данные для ЮЛ 1
        if 'df1' in locals() and df1 is not None:
            expenses_1 = calculate_expenses(df1)
            chart_data.append({
                'Юридическое лицо': 'ЮЛ 1',
                'Логистика': expenses_1.get('logistics', {}).get('amount', 0),
                'Хранение': expenses_1.get('storage', {}).get('amount', 0),
                'Прочие': expenses_1.get('other', {}).get('amount', 0)
            })
        
        # Данные для ЮЛ 2
        if 'df2' in locals() and df2 is not None:
            expenses_2 = calculate_expenses(df2)
            chart_data.append({
                'Юридическое лицо': 'ЮЛ 2',
                'Логистика': expenses_2.get('logistics', {}).get('amount', 0),
                'Хранение': expenses_2.get('storage', {}).get('amount', 0),
                'Прочие': expenses_2.get('other', {}).get('amount', 0)
            })
        
        # Общие данные
        chart_data.append({
            'Юридическое лицо': 'Общий',
            'Логистика': expenses_general.get('logistics', {}).get('amount', 0),
            'Хранение': expenses_general.get('storage', {}).get('amount', 0),
            'Прочие': expenses_general.get('other', {}).get('amount', 0)
        })
        ...
        if chart_data:
            ...
            # Добавляем столбцы для каждой категории расходов
            fig.add_trace(go.Bar(
                name='🚚 Логистика',
                x=chart_df['Юридическое лицо'],
                y=chart_df['Логистика'],
                marker_color='#1f77b4',
                text=[f"{x:,.0f} ₽" for x in chart_df['Логистика']],
                textposition='auto'
            ))
            ...
            fig.add_trace(go.Bar(
                ...
                y=chart_df['Хранение'],
                marker_color='#ff7f0e',
                text=[f"{x:,.0f} ₽" for x in chart_df['Хранение']],
                textposition='auto'
            ))
            ...
            fig.add_trace(go.Bar(
                ...
                y=chart_df['Прочие'],
                marker_color='#2ca02c',
                text=[f"{x:,.0f} ₽" for x in chart_df['Прочие']],
                textposition='auto'
            ))
            
            # Настройка графика
            fig.update_layout(
                title='Расходы по категориям и юридическим лицам',
                xaxis_title='Юридическое лицо',
                yaxis_title='Сумма (₽)',
                barmode='group',
                height=500,
                showlegend=True,
            ...
            for _, row in chart_df.iterrows():
                total_expenses = row['Логистика'] + row['Хранение'] + row['Прочие']
                summary_data.append({
                    'Юридическое лицо': row['Юридическое лицо'],
                    'Логистика (₽)': f"{row['Логистика']:,.0f}",
                    'Логистика (%)': f"{(row['Логистика']/total_expenses*100):.1f}%" if total_expenses > 0 else "0%",
                    'Хранение (₽)': f"{row['Хранение']:,.0f}",
                    'Хранение (%)': f"{(row['Хранение']/total_expenses*100):.1f}%" if total_expenses > 0 else "0%",
                    'Прочие (₽)': f"{row['Прочие']:,.0f}",
                    'Прочие (%)': f"{(row['Прочие']/total_expenses*100):.1f}%" if total_expenses > 0 else "0%",
                    'Итого (₽)': f"{total_expenses:,.0f}"
                })
            ...
            st.markdown("### 📈 График расходов по месяцам")
            ...
            # Данные для ЮЛ 1
            if 'df1' in locals() and df1 is not None:
                monthly_1 = calculate_monthly_expenses(df1)
                for month, expenses in monthly_1.items():
                    monthly_data.append({
                        'Месяц': month,
                        'Юридическое лицо': 'ЮЛ 1',
                        'Логистика': expenses.get('logistics', 0),
                        'Хранение': expenses.get('storage', 0),
                        'Прочие': expenses.get('other', 0),
                        'Всего': expenses.get('total', 0)
                    })
            
            # Данные для ЮЛ 2
            if 'df2' in locals() and df2 is not None:
                monthly_2 = calculate_monthly_expenses(df2)
                for month, expenses in monthly_2.items():
                    monthly_data.append({
                        'Месяц': month,
                        'Юридическое лицо': 'ЮЛ 2',
                        'Логистика': expenses.get('logistics', 0),
            ...
            if monthly_data:
                # Группируем по месяцам для общих данных
                monthly_general = {}
                for item in monthly_data:
                    month = item['Месяц']
                    if month not in monthly_general:
                        monthly_general[month] = {
                            'logistics': 0,
                            'storage': 0,
                            'other': 0,
                            'total': 0
                        }
                    monthly_general[month]['logistics'] += item['Логистика']
                    monthly_general[month]['storage'] += item['Хранение']
                    monthly_general[month]['other'] += item['Прочие']
                    monthly_general[month]['total'] += item['Всего']
                
                # Добавляем общие данные
                for month, expenses in monthly_general.items():
                    monthly_data.append({
                        'Месяц': month,
                        'Юридическое лицо': 'Общий',
                        'Логистика': expenses.get('logistics', 0),
                ...
                monthly_df['Месяц_число'] = pd.to_datetime(monthly_df['Месяц'], format='%Y-%m')
                monthly_df = monthly_df.sort_values('Месяц_число')
                
                # Создаем график
                fig_monthly = go.Figure()
                
                # Добавляем линии для каждого типа расходов
                for legal_entity in monthly_df['Юридическое лицо'].unique():
                    entity_data = monthly_df[monthly_df['Юридическое лицо'] == legal_entity]
                    
                    fig_monthly.add_trace(go.Scatter(
                        x=entity_data['Месяц'],
                        y=entity_data['Всего'],
                        mode='lines+markers',
                        name=f'{legal_entity} - Всего',
                        line=dict(width=3),
                        marker=dict(size=8)
                    ))
                ...
                # Настройка графика
                fig_monthly.update_layout(
                    title='Общие расходы по месяцам',
                    xaxis_title='Месяц',
                    yaxis_title='Сумма (₽)',
                    height=500,
                    showlegend=True,
                ...
                # Добавляем линии для логистики и хранения по каждому юридическому лицу
                for legal_entity in monthly_df['Юридическое лицо'].unique():
                    entity_data = monthly_df[monthly_df['Юридическое лицо'] == legal_entity]
                    ...
                    fig_logistics_storage.add_trace(go.Scatter(
                        x=entity_data['Месяц'],
                        y=entity_data['Логистика'],
                        mode='lines+markers',
                        name=f'{legal_entity} - 🚚 Логистика',
                    ...
                    fig_logistics_storage.add_trace(go.Scatter(
                        x=entity_data['Месяц'],
                        y=entity_data['Хранение'],
                        mode='lines+markers',
                        name=f'{legal_entity} - 📦 Хранение',
                ...
                # Настройка графика
                fig_logistics_storage.update_layout(
                    title='Расходы на логистику и хранение по месяцам',
                    xaxis_title='Месяц',
                    yaxis_title='Сумма (₽)',
                    height=500,
                    showlegend=True,
        ...
        with st.expander("🔮 Общий прогноз ROI с учетом остатков", expanded=False):
            ...
            # Получаем текущие данные для расчета
            total_invested_amount = total_invested_from_second_general
            
            # Инициализируем переменную для даты первого вложения
            first_investment_date = None
            if all_investments:
                first_investment_date = min(inv['date'] for inv in all_investments)
        ...
        if use_first_investment_date and all_investments:
            ...
            summary_avg_expenses = expenses_general.get('total_to_pay', {}).get('amount', 0) / summary_weeks if summary_weeks > 0 else 0
        else:
        ...
        <div class="total-card">
            ...
            <ul>
                ...
                <li><strong>Итого к оплате:</strong> {expenses_general.get('total_to_pay', {}).get('amount', 0):,.0f} ₽</li>
                <li><strong>Налог (7%):</strong> {tax_amount:,.0f} ₽</li>
                <li><strong>Итого к оплате (налог):</strong> {total_after_tax:,.0f} ₽</li>
                <li><strong>Общая сумма (Итого к оплате + расходы):</strong> {total_amount:,.0f} ₽</li>
                <li><strong>Все расходы:</strong> {total_expenses:,.0f} ₽</li>
