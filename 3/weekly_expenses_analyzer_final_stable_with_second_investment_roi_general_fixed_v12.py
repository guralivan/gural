...
def analyze_single_file_data(df, file_name, tab_prefix=""):
    ...
    # Отладочная информация для второго юридического лица
    if tab_prefix == "tab2_":
        st.info(f"🔍 **Отладка для ЮЛ 2:** Найдено юридическое лицо: '{legal_entity}'")
        if 'investment_data' in st.session_state:
            available_entities = list(st.session_state.investment_data.keys())
            st.info(f"🔍 **Доступные ключи вложений:** {available_entities}")
            if f"{legal_entity}_list" in st.session_state.investment_data:
                st.success(f"✅ Найдены вложения для '{legal_entity}'")
                # Показываем детали вложений
                investments_for_entity = st.session_state.investment_data[f"{legal_entity}_list"]
                st.info(f"🔍 **Детали вложений:** {len(investments_for_entity)} вложений")
                for i, inv in enumerate(investments_for_entity):
                    st.info(f"  - Вложение #{i+1}: {inv['amount']:,.0f} ₽ от {inv['date']}")
            else:
                st.warning(f"⚠️ Вложения для '{legal_entity}' не найдены")
    ...
    with col3:
        ...
        if investments_list and len(investments_list) > 0:
            # Отладочная информация для второго юридического лица
            if tab_prefix == "tab2_":
                st.info(f"🔍 **Отладка фильтрации:** Всего вложений: {len(investments_list)}")
                st.info(f"🔍 **Параметры фильтрации:** start_date={start_date}, end_date={end_date}, roi_calculation_date={roi_calculation_date}")
            ...
            for inv in investments_list:
                ...
                if safe_date_compare(inv['date'], roi_calculation_date) or inv['date'] == roi_calculation_date:
                    ...
                    if (safe_date_compare(inv['date'], start_date) or inv['date'] == start_date) and (safe_date_compare(end_date, inv['date']) or inv['date'] == end_date):
                        ...
                        # Отладочная информация для второго юридического лица
                        if tab_prefix == "tab2_":
                            st.success(f"✅ **Добавлено вложение:** {inv['amount']:,.0f} ₽ от {inv['date']}")
                    else:
                        # Отладочная информация для второго юридического лица
                        if tab_prefix == "tab2_":
                            st.warning(f"⚠️ **Вложение исключено (не в периоде):** {inv['amount']:,.0f} ₽ от {inv['date']}")
                else:
                    # Отладочная информация для второго юридического лица
                    if tab_prefix == "tab2_":
                        st.warning(f"⚠️ **Вложение исключено (до ROI даты):** {inv['amount']:,.0f} ₽ от {inv['date']}")
            
            # Отладочная информация для второго юридического лица
            if tab_prefix == "tab2_":
                st.info(f"🔍 **Результат фильтрации:** {len(period_investments)} вложений в периоде")
                if period_investments:
                    total_period_amount = sum(inv['amount'] for inv in period_investments)
                    st.success(f"✅ **Сумма вложений в периоде:** {total_period_amount:,.0f} ₽")
                else:
                    st.error(f"❌ **Нет вложений в периоде!**")
    ...
    with tab3:
        ...
        if all_investments:
            ...
            if use_second_investment_date_general and len(all_investments) > 1:
                ...
                # Рассчитываем сумму вложений с даты второго вложения
                investments_from_second_general = [inv for inv in all_investments if inv['date'] >= roi_calculation_date_general]
                total_invested_from_second_general = sum(inv['amount'] for inv in investments_from_second_general)
            else:
                roi_calculation_date_general = first_investment_date  # Первое вложение
                total_invested_from_second_general = total_invested  # Используем общую сумму
        ...
        if all_investments:
            ...
            # Добавляем вложения с даты расчета ROI (отрицательные потоки)
            for inv in all_investments:
                if inv['date'] >= roi_calculation_date_general:
                    cashflows.append(-inv['amount'])  # Отрицательные = вложения
                    dates.append(inv['date'])
        ...
        with col3:
            st.metric(
                ...
                delta=f"{len([inv for inv in all_investments if inv['date'] >= roi_calculation_date_general])} вложений (с {roi_calculation_date_general.strftime('%d.%m.%Y')})"
            )
