...
def analyze_single_file_data(df, file_name, tab_prefix=""):
    ...
    if has_investment and saved_amount > 0:
        ...
        if investments_list and len(investments_list) > 0:
            ...
            if start_date > first_investment_date:
                # Если дата выбрана после вложения, не учитываем вложение в расчетах
                st.warning(f"⚠️ Выбранная дата ({start_date.strftime('%d.%m.%Y')}) позже даты первого вложения ({first_investment_date.strftime('%d.%m.%Y')}). Вложения не учитываются в расчетах.")
                roi = 0
                profit_after_tax = total_after_tax  # Показываем только доход без учета вложений
            else:
                # Фильтруем данные с даты первого вложения
    ...
    with col3:
        ...
        if has_investment and saved_amount > 0:
            ...
            if investments_list and len(investments_list) > 0:
                first_investment_date = min(inv['date'] for inv in investments_list)
                if start_date > first_investment_date:
                    # Если дата выбрана после вложения, показываем 0 вложений
                    st.metric(
                        label="💰 Итого вложено",
                        value="0 ₽",
                        delta="Вложения не учитываются (дата после вложения)"
                    )
                else:
                    # KPI Итого вложено
                    st.metric(
            ...
            if start_date > first_investment_date:
                # Если дата выбрана после вложения, ROI за выбранный период = 0
            ...
            if investments_list and len(investments_list) > 0:
                ...
                # Проверяем, не выбрана ли дата после вложения
                if start_date > first_investment_date:
                    # Если дата выбрана после вложения, банковский доход = 0
                    bank_income = 0
                    bank_roi = 0
                else:
                    # Рассчитываем количество дней с первого вложения до конца выбранного периода
                    end_date_datetime = pd.to_datetime(end_date)
                    days_invested = (end_date_datetime - pd.to_datetime(first_investment_date)).days
            ...
            if investments_list and len(investments_list) > 0:
                # Проверяем, не выбрана ли дата после вложения
                if start_date > first_investment_date:
                    # Если дата выбрана после вложения, XIRR = 0
                    xirr = 0
                else:
