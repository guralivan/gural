...
def analyze_single_file_data(df, file_name, tab_prefix=""):
    ...
    if has_investment and investments_list:
        st.checkbox(
            ...
            value=st.session_state.get(f"{tab_prefix}roi_first_date_{legal_entity}", False),
    ...
    if 'Итого к оплате' in filtered_df.columns:
        ...
        fig_monthly.update_layout(
            height=500,
            bargap=0.2,  # Увеличиваем промежутки между столбцами
            bargroupgap=0.1,  # Увеличиваем промежутки между группами
        ...
        fig_monthly.update_yaxes(
            tickformat=",",
            title="Сумма (₽)",
            gridcolor='rgba(128,128,128,0.2)',  # Светлая сетка
            zeroline=False
        )
        ...
        fig_monthly.update_traces(
            marker_color='#A23B72',  # Красивый фиолетовый цвет
            opacity=0.9,  # Увеличиваем непрозрачность
            texttemplate='%{text:,.0f}',  # Формат текста на столбцах
            textposition='outside',  # Позиция текста
            textfont_size=11,  # Размер шрифта текста
            hovertemplate='<b>Месяц:</b> %{x}<br><b>Сумма:</b> %{y:,.0f} ₽<extra></extra>'  # Улучшенный шаблон при наведении
        )
    ...
    if 'Итого к оплате' in filtered_df.columns:
        ...
        # Создаем форматированные метки для оси X с месяцем, неделей и годом
        weekly_data['Месяц_Неделя_Год'] = weekly_data['Дата формирования'].dt.strftime('%b W%U %Y')
        
        # Создаем столбчатый график
        fig_total_bar = px.bar(
            x=weekly_data['Месяц_Неделя_Год'],
            y=amounts_rounded,
            title='"Итого к оплате" по неделям',
            labels={'x': 'Месяц-Неделя-Год', 'y': 'Итого к оплате (₽)'},
        ...
        fig_total_bar.update_layout(
            height=500,
            bargap=0.2,  # Увеличиваем промежутки между столбцами для лучшей читаемости
            bargroupgap=0.1,  # Увеличиваем промежутки между группами
            showlegend=False,
            plot_bgcolor='rgba(0,0,0,0)',  # Прозрачный фон
            paper_bgcolor='rgba(0,0,0,0)'  # Прозрачный фон
        ...
        fig_total_bar.update_traces(
            marker_color='#2E86AB',  # Красивый синий цвет
            opacity=0.9,  # Увеличиваем непрозрачность
            texttemplate='%{text:,.0f}',  # Формат текста на столбцах
            textposition='outside',  # Позиция текста
            textfont_size=10,  # Размер шрифта текста
            hovertemplate='<b>Неделя:</b> %{x}<br><b>Сумма:</b> %{y:,.0f} ₽<extra></extra>'  # Улучшенный шаблон при наведении
        )
    ...
    if 'Итого к оплате' in filtered_df.columns:
        ...
        if len(years_present) >= 2:
            ...
            fig_comparison.update_traces(
                ...
                textfont_size=9,  # Уменьшаем размер шрифта для лучшей читаемости
                hovertemplate='<b>Неделя:</b> %{x}<br><b>Год:</b> %{fullData.name}<br><b>Сумма:</b> %{y:,.0f} ₽<extra></extra>'
            )
    ...
    with tab3:
        ...
        if all_investments:
            ...
            use_first_investment_date = st.checkbox(
                ...
                value=st.session_state.get("general_kpi_first_date", False),
