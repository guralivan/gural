...
def main():
    ...
    with tab3:
        ...
        if not cost_products:
            ...
            total_china_cost_sum = 0
            total_logistics_china_sum = 0
            
            for product in cost_products:
                results = product['results']
                total_cost_sum += results['total_cost']
                total_quantity_sum += product['quantity']
                total_china_cost_sum += results['price_rub_china']
                ...
                # Добавляем к общей сумме логистики
                total_logistics_china_sum += logistics_total
            ...
            if cost_table_data:
                cost_table_data.append({
                    ...
                    'Стоимость в Китае общая (RUB)': format_currency(total_china_cost_sum),
                    'Доставка в России (USD)': '',
                    'Логистика из Китая на ед.': '',
                    'Логистика из Китая общая': format_currency(total_logistics_china_sum),
                    'Вес (г)': '',
