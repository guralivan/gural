...
def load_param_values_from_file():
    ...
    param_files = glob.glob("param_values_*.json")
        ...
        try:
            ...
            for param_name, sku_values in file_params.items():
                ...
                if param_name and isinstance(sku_values, dict):
                    ...
                    # Собираем все уникальные значения для param_options
    ...
    if all_param_values:
        ...
        if all_param_options:
            st.session_state["param_options"] = all_param_options
...
if "table_loaded" not in st.session_state:
    try:
        ...
        if os.path.exists("table_cache.json"):
            ...
            # Фильтруем невалидные параметры (артикулы и т.д.)
            st.session_state["param_options"] = {
    ...
    with col_save1:
        ...
        with tab2:
            ...
            with st.expander("📝 Редактор параметров", expanded=False):
                ...
                if param_values:
                    for param_name in param_values.keys():
                        # Не синхронизируем удаленные параметры
                        if param_name not in deleted_params and param_name not in param_options:
            ...
            if analysis_mode == "Одиночный анализ":
                ...
                if not mass_analysis_results:
                    ...
                    if saved_results:
                        ...
                        st.session_state["mass_analysis_results"] = saved_results
                        # Автоматически применяем параметры из загруженных результатов
                ...
                if mass_analysis_results:
                    ...
                    filter_options = ["Все результаты", "✅ Успешно", "⚠️ Неопределено", "❌ Ошибки", "⏭️ Пропущено", "⏳ Необработан"]
                ...
                if source_df is None or source_df.empty or "Артикул" not in source_df.columns:
                    ...
                    # Получаем список артикулов из таблицы
                                    ...
                                    if remaining_skus:
                                        ...
                                        for idx, sku in enumerate(remaining_skus):
                                            ...
                                            try:
                                                ...
                                                if params:
                                                    ...
                                                    if existing_result_idx is not None:
                                                        ...
                                                        old_status = mass_results[existing_result_idx].get("Статус", "")
                                                        mass_results[existing_result_idx] = result_row
                                                        # Если старый статус был ошибкой, а новый успешный - не нужно менять счетчик
                                                    else:
                                                    ...
                                                    # Если результат уже есть - обновляем, иначе добавляем
                                                    if existing_result_idx is not None:
                                                        # Обновляем только если предыдущий статус был ошибкой или неопределен
                                                        old_status = mass_results[existing_result_idx].get("Статус", "")
                                                        if not old_status.startswith("✅"):
                                                            mass_results[existing_result_idx] = result_row
                                                    else:
                                                        mass_results.append(result_row)
                                            ...
                                            if mass_results:
                                                ...
                                                with results_table_placeholder.container():
                                                    st.markdown(f"**📋 Обработано: {len(mass_results)} товаров** (✅ Успешно: {current_success_count}, ❌ Ошибок: {current_error_count})")
                                                    st.dataframe(results_df, use_container_width=True, height=400)
            ...
            with col_param1:
                ...
                # Выбор категории/иерархии параметра
                param_category = st.selectbox(
                    "Категория параметра:",
                    options=["Обычный параметр", "Иерархический (Тип/Комплект/Рукав/Ворот)", "Визуальный (Цвет/Принт/Строчка)"],
                    index=0,
                    help="Иерархические параметры влияют на структуру анализа. Визуальные параметры описывают внешний вид товара.",
                    key="param_category_select"
                )
                ...
                if st.button("➕ Добавить параметр", type="primary", use_container_width=True):
                    if param_name and param_name.strip():
                        try:
                            ...
                            # Проверяем, не существует ли уже такой параметр
                            if param_name.strip() in param_options:
                                st.error(f"❌ Параметр '{param_name.strip()}' уже существует!")
                            else:
                                # Добавляем параметр
                                param_options[param_name.strip()] = options
                                if param_name.strip() not in param_values:
                                    param_values[param_name.strip()] = {}
                                
                                st.session_state["param_options"] = param_options
                                st.session_state["param_values"] = param_values
                                
                                # Сохраняем историю изменений
                                for sku in df["Артикул"].dropna().unique():
                                    sku_str = str(sku).replace(".0", "")
                                    add_param_history(sku_str, param_name.strip(), "", "Добавлен новый параметр")
                                
                                if save_param_values_to_file():
                                    st.success(f"✅ Параметр '{param_name.strip()}' добавлен! ({len(options)} вариантов)")
                                    # Очищаем поля
                                    if "temp_param_name" in st.session_state:
                                        del st.session_state["temp_param_name"]
                                    if "temp_param_options" in st.session_state:
                            ...
                            st.error(f"❌ Ошибка при добавлении параметра: {e}")
                    else:
                        st.warning("⚠️ Введите название параметра")
            ...
            with col_clear:
                ...
                if param_category == "Иерархический (Тип/Комплект/Рукав/Ворот)":
                    st.info("ℹ️ Иерархические параметры: Тип, Комплект, Рукав, Ворот")
                elif param_category == "Визуальный (Цвет/Принт/Строчка)":
                    st.info("ℹ️ Визуальные параметры: Цвет, Принт, Логотип, Строчка, Шов")
                ...
                if st.button(button_text, type=button_type):
                    if param_name:
                        try:
                            ...
                            # Получаем param_values и param_options из session_state
                            if "param_values" not in st.session_state:
                                st.session_state["param_values"] = {}
                            if "param_options" not in st.session_state:
                                st.session_state["param_options"] = {}
                            ...
                            param_options_dict = st.session_state["param_options"]
                            
                            # Создаем или обновляем параметр (даже если вариантов нет)
                            if param_name not in param_values:
                                param_values[param_name] = {}
                            
                            old_count = len(param_options_dict.get(param_name, []))
                            param_options_dict[param_name] = options
                            new_count = len(options)
                            
                            # Обновляем session_state
                            st.session_state["param_values"] = param_values
                            st.session_state["param_options"] = param_options_dict
                            ...
                            if param_exists:
                                ...
                                category_info = ""
                                if param_category == "Иерархический (Тип/Комплект/Рукав/Ворот)":
                                    category_info = " (иерархический)"
                                elif param_category == "Визуальный (Цвет/Принт/Строчка)":
                                    category_info = " (визуальный)"
                                
                                if new_count > 0:
                                    st.success(f"✅ Параметр '{param_name}'{category_info} добавлен с вариантами: {', '.join(options[:5])}{'...' if len(options) > 5 else ''}")
                                else:
                                    st.success(f"✅ Параметр '{param_name}'{category_info} добавлен (без ограничений по вариантам - нейросеть определит значение самостоятельно)")
                        ...
                        st.warning("Заполните название параметра")
            ...
            with col_clear:
                ...
                # Кнопка для очистки невалидных параметров (артикулов и т.д.)
                if st.button("🧹 Очистить невалидные параметры"):
                    if "param_options" in st.session_state:
                        invalid_params = []
                        ...
                        # Также удаляем из param_values
                        if "param_values" in st.session_state:
                            for param_name in invalid_params:
                                if param_name in st.session_state["param_values"]:
                                    del st.session_state["param_values"][param_name]
                        
                        if invalid_params:
                            if save_param_values_to_file():
                                st.success(f"✅ Удалено {len(invalid_params)} невалидных параметров: {', '.join(invalid_params)}")
                                st.rerun()
                            else:
                                st.warning(f"⚠️ Удалено {len(invalid_params)} невалидных параметров, но изменения не сохранены в файл")
                        else:
                            st.info("ℹ️ Невалидных параметров не найдено")
                            ...
                            for index, row in edited_df.iterrows():
                                ...
                                for param in all_params:
                                    if param in row and row[param]:
                                        if param not in param_values:
                                            param_values[param] = {}
                                        old_value = param_values[param].get(sku, None)
                                        new_value = str(row[param])
                                        if old_value != new_value:
                                            # Используем функцию с сохранением истории
                                            save_param_value(str(sku), param, new_value, save_history=True)
                                            changes_made = True
                                    elif param in param_values and sku in param_values[param]:
                                        # Удаляем пустые значения
                                        if not row.get(param):
        ...
        with tab3:
            ...
            if not param_values and not param_options:
                ...
                # Разделяем параметры на категории
                hierarchical_params_list = [p for p in hierarchy_params if p in available_params]
                visual_params_list = [p for p in available_params if p in visual_params or any(vp in p for vp in visual_params)]
                other_params_list = [p for p in available_params if p not in hierarchy_params and p not in visual_params_list]
                ...
                for sku in df["Артикул"].astype(str).str.replace(".0", "").unique():
                    ...
                    # Собираем все параметры для этого артикула
                    for param_name in available_params:
                        if param_name in param_values and sku_str in param_values[param_name]:
                            value = param_values[param_name][sku_str]
                            if value and str(value).strip():
                                ...
                                # Если хотя бы один параметр пустой, комбинация неполная
                                combination_valid = False
                                break
                        else:
                            # Если параметр отсутствует, комбинация неполная
                            combination_valid = False
                            break
                    
                    if combination_valid and combination_parts:
                        combination_key = " | ".join(sorted(combination_parts))  # Сортируем для единообразия
                ...
                if all_combinations:
                    ...
                    if combination_analytics:
                        ...
                        # Добавляем рейтинг к каждой комбинации
                        ...
                        # Показываем интерфейс для исключения значений параметров
                        with st.expander("🚫 Исключить значения параметров из анализа", expanded=False):
                            ...
                            # Для каждого параметра создаем мультиселект
                            excluded_selected = {}
                            ...
                            total_excluded = sum(len(values) for values in excluded_selected.values())
                            if total_excluded > 0:
                                excluded_list = []
                                for param_name, values in excluded_selected.items():
                                    if values:
                                        excluded_list.append(f"{param_name}: {', '.join(values)}")
                                st.info(f"ℹ️ Исключено значений параметров: {total_excluded}")
                                for item in excluded_list:
                                    st.write(f"  • {item}")
                        
                        # Фильтруем комбинации по исключенным значениям параметров
                        ...
                        filtered_combinations = [
                            combo for combo in combination_analytics 
                            if not should_exclude_combination(combo["Комбинация"])
                        ]
                        
                        # Показываем топ-10 из отфильтрованных комбинаций
                        top_10_combinations = filtered_combinations[:10]
                        
                        # Форматируем данные для отображения
                        display_data = []
                        for idx, combo in enumerate(top_10_combinations, 1):
                            display_data.append({
                                "Место": f"#{idx}",
                        ...
                        if top_10_combinations:
                            top_10_df = pd.DataFrame(display_data)
                            st.dataframe(top_10_df, use_container_width=True, hide_index=True)
                            
                            # Показываем лучшую комбинацию отдельно
                            best_combo = top_10_combinations[0]
                            st.success(f"🥇 **Лучшая комбинация**: {best_combo['Комбинация']}")
                            st.markdown(f"**Рейтинг:** {best_combo['Рейтинг']:.4f} | **Выручка на 1 артикул:** ₽{best_combo['Выручка на 1 артикул']:,.0f}".replace(",", " "))
                            
                            # Добавляем возможность просмотра товаров для каждой комбинации
                            st.divider()
                            st.subheader("📦 Детали комбинации")
                            
                            # Выбор комбинации для просмотра
                            combo_options = [f"#{idx} - {combo['Комбинация']}" for idx, combo in enumerate(top_10_combinations, 1)]
                            ...
                            if selected_combo_idx:
                                ...
                                selected_idx = int(selected_combo_idx.split(" - ")[0].replace("#", "")) - 1
                                selected_combo = top_10_combinations[selected_idx]
                                ...
                                if selected_combo_key in all_combinations:
                                    ...
                                    if not combo_products_df.empty:
                                        ...
                                        with col4:
                                            avg_pos = combo_products_df["Позиция в выдаче"].mean() if "Позиция в выдаче" in combo_products_df.columns else 0
                                            st.metric("Средняя позиция", f"{avg_pos:.1f}")
                                    ...
                                    st.warning("⚠️ Комбинация не найдена в данных")
                        else:
