...
def load_additional_data(uploaded_file):
    ...
    try:
        ...
        if uploaded_file.name.endswith('.xlsx') or uploaded_file.name.endswith('.xls'):
            ...
            # Сначала проверяем доступные листы
            try:
                excel_file = pd.ExcelFile(uploaded_file)
                available_sheets = excel_file.sheet_names
                st.info(f"📋 Доступные листы в файле: {', '.join(available_sheets)}")
                
                # Ищем подходящий лист
                target_sheet = None
                for sheet in available_sheets:
                    if 'товар' in sheet.lower() or 'product' in sheet.lower() or sheet.lower() == 'товары':
                        target_sheet = sheet
                        break
                ...
                if target_sheet is None:
                    ...
                    st.warning(f"⚠️ Лист 'Товары' не найден, используем первый лист: {target_sheet}")
                else:
                    st.info(f"✅ Найден подходящий лист: {target_sheet}")
                
            except Exception as e:
                st.error(f"❌ Ошибка при чтении структуры файла: {e}")
                return None
            ...
            try:
                ...
                try:
                    ...
                    st.error(f"❌ Не удалось загрузить файл. Ошибки:")
                    st.error(f"   • С заголовком на строке 1: {str(e1)}")
                    st.error(f"   • С обычными заголовками: {str(e2)}")
                    st.error(f"   • Проверьте, что файл не поврежден и содержит данные")
                    return None
            ...
            # Дополнительная проверка: если в файле нет нужных столбцов, пробуем другие варианты
            if len(df.columns) < 3:  # Минимальное количество столбцов для анализа
                st.warning("⚠️ Мало столбцов в файле, пробуем альтернативные способы загрузки...")
                try:
                    # Пробуем загрузить без указания листа (первый лист)
                    df_alt = pd.read_excel(uploaded_file, header=0)
                    if len(df_alt.columns) > len(df.columns):
                        df = df_alt
                        st.info("✅ Файл загружен альтернативным способом")
                except Exception as e3:
                    st.error(f"❌ Альтернативная загрузка также не удалась: {e3}")
                    return None
...
if uploaded_file is not None:
    ...
    try:
        ...
        st.sidebar.error(f"❌ Ошибка при обработке файла: {str(e)}")
        st.error(f"Детали ошибки: {e}")
        st.info("💡 Попробуйте:")
        st.info("   • Проверить, что файл не поврежден")
        st.info("   • Убедиться, что файл содержит данные")
        st.info("   • Попробовать другой файл")
...
if df is not None:
    ...
    if orders_col and sales_col:
        ...
        # Порядок: Средняя цена -> Реклама -> ДРР -> Заказ план -> Продажа план -> Процент выкупа -> Рентабельность факт -> Рентабельность план -> Прибыль на ед. -> Прибыль
        
        # Добавляем названия для новых строк в правильном порядке (сначала базовые, потом зависимые)
        # Порядок: Средняя цена -> Реклама -> ДРР -> Заказ план -> Продажа план -> Процент выкупа -> Рентабельность факт -> Рентабельность план -> Прибыль на ед. -> Прибыль
        
        # Проверяем, есть ли уже "Процент выкупа" в index_names из исходного файла
        has_buyout_percent = any('процент выкупа' in name.lower() or 'конверсия' in name.lower() for name in index_names)
        
        # Добавляем только те строки, которых еще нет
        additional_index_names = ["Средняя цена", "Реклама", "ДРР", "Заказ план", "Продажа план"]
        
        # Добавляем "Процент выкупа" только если его нет в исходных данных
        if not has_buyout_percent:
            additional_index_names.append("Процент выкупа")
        
        additional_index_names.extend(["Рентабельность факт", "Рентабельность план", "Прибыль на ед.", "Прибыль"])
        
        index_names.extend(additional_index_names)
        ...
        for idx_name in index_names:
            if idx_name == "Средняя цена":
                ...
                additional_rows.append(row.to_frame().T)
            elif idx_name == "Процент выкупа":
                # Расчет процента выкупа: (Выкупили, шт / Заказали, шт) * 100
                values = []
                for col in pivot_data.columns:
                    if col == "Общие по месяцам":
                        values.append(0.0)  # Будет рассчитано позже
                        ...
                        buyout_values = []
                        for week in month_weeks:
                            # Получаем данные о заказах и выкупах
                            orders_count = 0.0
                            sales_count = 0.0
                            
                            # Ищем столбцы с заказами и выкупами
                            if orders_col and orders_col in pivot_data.index:
                                orders_count = pivot_data.loc[orders_col, week] if pd.notna(pivot_data.loc[orders_col, week]) else 0.0
                            if sales_col and sales_col in pivot_data.index:
                                sales_count = pivot_data.loc[sales_col, week] if pd.notna(pivot_data.loc[sales_col, week]) else 0.0
                            ...
                            buyout_values.append(buyout_percent)
                        
                        # Среднее по неделям месяца
                        if buyout_values:
                            avg_buyout = sum(buyout_values) / len(buyout_values)
                        else:
                            avg_buyout = 0.0
                        values.append(avg_buyout)
                    else:
                        ...
                        # Ищем столбцы с заказами и выкупами
                        if orders_col and orders_col in pivot_data.index:
                            orders_count = pivot_data.loc[orders_col, col] if pd.notna(pivot_data.loc[orders_col, col]) else 0.0
                        if sales_col and sales_col in pivot_data.index:
                            sales_count = pivot_data.loc[sales_col, col] if pd.notna(pivot_data.loc[sales_col, col]) else 0.0
                        ...
                        values.append(buyout_percent)
                
                row = pd.Series(values, index=pivot_data.columns)
                row.name = "Процент выкупа"
                additional_rows.append(row.to_frame().T)
                            ...
                            if week_col != "Общие по месяцам" and not (week_col.startswith(("2024.", "2023.", "2022.", "2025.")) and '(' not in week_col):
                                ...
                                if avg_price > 0:
                                    ...
                                    if profit_per_unit == 0.0 and avg_price > 0:
                                        ...
                                        delivery_cost = logistics_cost
                                        tax_per_unit = avg_price * 0.07  # Налог 7%
                                        profit_per_unit = avg_price - cost_price - commission_amount - advertising_cost - delivery_cost - tax_per_unit - storage_cost
        ...
        # Функция для безопасного получения скалярного значения
        def get_scalar_value(value):
            """Безопасно извлекает скалярное значение из pandas объекта"""
            if hasattr(value, 'iloc'):
                # Если это Series, берем первое значение
                return value.iloc[0] if len(value) > 0 else 0.0
            elif hasattr(value, 'item'):
                # Если это numpy scalar, конвертируем в Python scalar
                try:
                    return value.item()
                except:
                    return float(value)
        ...
        for col in formatted_data.columns:
            for idx in formatted_data.index:
                if idx not in ['Реклама', 'ДРР', 'Средняя цена', 'Заказ план', 'Продажа план', 'Процент выкупа', 'Прибыль на ед.', 'Конверсия в корзину, %', 'Конверсия в заказ, %', 'Рентабельность факт', 'Рентабельность план', 'Прибыль'] and 'Заказали на сумму' not in idx and 'Выкупили на сумму' not in idx:  # Не форматируем эти строки
                    ...
                    original_value = get_scalar_value(pivot_data.loc[idx, col])
        ...
        with tab1:
            ...
            # Рассчитываем накопленные значения за ВСЕ периоды с данными
            total_orders = 0
            total_sales = 0
            total_revenue = 0
            total_sales_plan = 0
            ...
            # Собираем все недели с данными до текущей даты
            weeks_with_data = []
            for col in weekly_columns:
                if "(нед." in col:
                    try:
                        # Извлекаем номер недели из названия столбца
                        week_num = int(col.split("нед. ")[1].split(")")[0])
                        
                        # Проверяем, что неделя не превышает текущую
                        if week_num > current_week:
                            continue
                        
                        # Проверяем, есть ли данные в этой неделе
                        has_orders = False
                        has_sales = False
                        
                        if "Заказали, шт" in pivot_data.index:
                            orders = pivot_data.loc["Заказали, шт", col]
                            if pd.notna(orders) and orders > 0:
                                has_orders = True
                                total_orders += orders
                        
                        if "Выкупили, шт" in pivot_data.index:
                            sales = pivot_data.loc["Выкупили, шт", col]
                            if pd.notna(sales) and sales > 0:
                                has_sales = True
                                total_sales += sales
                        
                        if "Заказали на сумму" in pivot_data.index:
                            revenue = pivot_data.loc["Заказали на сумму", col]
                            if pd.notna(revenue) and revenue > 0:
                                total_revenue += revenue
                        
                        # Суммируем план продаж из таблицы
                        if "Продажа план" in pivot_data.index:
                            plan_sales = pivot_data.loc["Продажа план", col]
                            if pd.notna(plan_sales) and plan_sales > 0:
                                total_sales_plan += plan_sales
                        
                        # Суммируем план заказов из session state
                        if col in st.session_state.orders_plan_values:
                            plan_orders = st.session_state.orders_plan_values[col]
                            if plan_orders > 0:
                                total_orders_plan += plan_orders
                        
                        # Если есть заказы или продажи, добавляем неделю
                        if has_orders or has_sales:
                            weeks_with_data.append(col)
                            
                    except (ValueError, IndexError):
                        continue
            
            # Показываем информацию о найденных данных
            if len(weeks_with_data) > 0:
                st.info(f"📊 Найдено {len(weeks_with_data)} недель с данными: {weeks_with_data[0]} - {weeks_with_data[-1]}")
                
                # Показываем период анализа (с 25 недели по текущую дату)
                st.caption(f"📅 Период анализа: с 25 недели по {current_week} неделю (2025 год)")
                
                # Показываем планы за период
                if total_orders_plan > 0 or total_sales_plan > 0:
                    st.caption(f"📋 План заказов за период: {total_orders_plan:,.0f}")
                    st.caption(f"📋 План продаж за период: {total_sales_plan:,.0f}")
            else:
                st.warning("⚠️ Не найдено недель с данными для расчета KPI")
                st.stop()
            ...
            with col1:
                # Заказы (накопленные) с сравнением с планом
                if total_orders_plan > 0:
                    orders_difference = total_orders - total_orders_plan
                    orders_percentage = (orders_difference / total_orders_plan) * 100
                    
                    # Определяем статус выполнения плана заказов
                    if orders_percentage > 10:
                        orders_status = "✅ Опережает план"
                    elif orders_percentage < -10:
                        orders_status = "❌ Отстает от плана"
                    else:
                        orders_status = "⚠️ Близко к плану"
                    
                    st.metric(
                        "📦 Заказы (накопленные)",
                        f"{total_orders:,.0f}",
                        delta=f"{orders_difference:+.0f}",
                        help=f"План: {total_orders_plan:,.0f} | {orders_status}"
                    )
                    
                    # Показываем процент выполнения плана заказов
                    st.caption(f"📊 Выполнение плана заказов: {orders_percentage:+.1f}%")
                else:
                    st.metric(
                        "📦 Заказы (накопленные)",
                        f"{total_orders:,.0f}",
            ...
            with col2:
                # Продажи (накопленные) с сравнением с планом
                if total_sales_plan > 0:
                    ...
                    if sales_percentage > 10:
                        status = "✅ Опережает план"
                        color = "green"
                    elif sales_percentage < -10:
                        status = "❌ Отстает от плана"
                        color = "red"
                    else:
                        status = "⚠️ Близко к плану"
                        color = "orange"
                    ...
                    st.metric(
                        ...
                        help=f"План: {total_sales_plan:,.0f} | {status}"
                    )
                    
                    # Показываем процент выполнения
                    st.caption(f"📊 Выполнение плана: {sales_percentage:+.1f}%")
                else:
                    st.metric(
                        "💰 Продажи (накопленные)",
                        f"{total_sales:,.0f}",
            ...
            with col3:
                ...
                if total_orders > 0:
                    ...
                    st.metric(
                        "📊 Конверсия (общая)",
                        f"{conversion:.1f}%",
                        help=f"Процент заказов, которые стали продажами"
                    )
                else:
            ...
            with col4:
                ...
                if total_revenue > 0:
                    st.metric(
                        "💵 Выручка (накопленная)",
                        f"{total_revenue:,.0f} ₽",
                        help=f"Общая выручка за весь период"
                    )
                else:
                    st.metric("💵 Выручка (накопленная)", "Нет данных")
            ...
            # Фильтруем столбцы для расчета KPI - включаем ВСЕ недели с данными
            # НЕ учитываем настройку скрытия недель для KPI
            ...
            for col in weekly_columns:
                ...
                if "(нед." in col:
                    try:
                        ...
                        # ИСПРАВЛЕНИЕ: НЕ учитываем hide_weeks_before для расчета KPI
                        # Проверяем, есть ли данные в этой неделе
                        has_data = False
                        if "Заказали, шт" in pivot_data.index:
                            orders = pivot_data.loc["Заказали, шт", col]
                        ...
                        # Включаем неделю, если в ней есть данные
                        if has_data:
                            filtered_columns.append(col)
                    except (ValueError, IndexError):
                        continue
        ...
        with tab2:
            ...
            # Проверяем, есть ли данные в текущей неделе
            ...
            if current_week_col in pivot_data.columns and "Заказали, шт" in pivot_data.index:
                ...
                if pd.notna(actual_orders) and actual_orders > 0:
                    has_current_week_data = True
            
            # Если в текущей неделе нет данных, ищем последнюю неделю с данными
            kpi_week_col = current_week_col
            if not has_current_week_data:
                # Ищем последнюю неделю с данными (в обратном порядке)
                for col in reversed(pivot_data.columns):
                    if "нед." in col and "Заказали, шт" in pivot_data.index:
                        try:
                            orders = pivot_data.loc["Заказали, шт", col]
                            if pd.notna(orders) and orders > 0:
                                kpi_week_col = col
                                st.info(f"📊 Показываем KPI за последнюю неделю с данными: {col}")
                                break
                        except:
                            continue
            
            # Получаем план для выбранной недели
            plan_orders = st.session_state.orders_plan_values.get(kpi_week_col, 0)
            plan_rentability = st.session_state.rentability_plan_values.get(kpi_week_col, 0)
            
            # Показываем информацию о том, какая неделя используется для расчета
            if kpi_week_col != current_week_col:
                st.info(f"📊 KPI рассчитывается за неделю: {kpi_week_col} (в текущей неделе {current_week_col} нет данных)")
            else:
                st.info(f"📊 KPI рассчитывается за текущую неделю: {kpi_week_col}")
