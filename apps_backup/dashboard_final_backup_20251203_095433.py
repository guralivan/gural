...
- Если параметр определить невозможно, укажи пустую строку ""
    ...
    with col_save1:
        ...
        with tab2:
            ...
            if "Артикул" in df.columns and len(df) > 0:
                ...
                # Проверяем, какие артикулы уже имеют параметры
                skus_with_params = set()
                for param_name in param_values.keys():
                    skus_with_params.update(param_values[param_name].keys())
                
                # Разделяем на группы
                skus_without_params = [sku for sku in all_skus if str(sku) not in skus_with_params]
                skus_with_params_list = [sku for sku in all_skus if str(sku) in skus_with_params]
                
                st.info(f"📊 Всего товаров в таблице: **{len(all_skus)}** | "
                       f"С параметрами: **{len(skus_with_params_list)}** | "
                ...
                if analysis_mode == "Анализировать все товары":
                    ...
                    selected_skus = skus_without_params
                    if selected_skus:
                        st.write(f"✅ Будет проанализировано: **{len(selected_skus)}** товаров без параметров")
                    else:
                        st.success("✅ Все товары уже имеют параметры!")
                    ...
                    selected_skus = st.multiselect(
                        ...
                        options=all_skus,
                        default=skus_without_params[:10] if skus_without_params else [],
                        help="Выберите товары, которые нужно проанализировать"
                    )
                    st.write(f"✅ Выбрано товаров: **{len(selected_skus)}**")
