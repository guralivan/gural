...
def save_param_values_to_file():
    ...
    if param_values and current_file:
        try:
            ...
            # Сохраняем параметры с информацией о файле
            param_data = {
...
def load_param_values_from_file():
    """Загружает параметры из файла, привязанные к текущему файлу"""
    current_file = st.session_state.get("cached_file_name", None)
    
    if current_file:
        try:
            # Пытаемся загрузить параметры для текущего файла
            registry_file = "file_params_registry.json"
            if os.path.exists(registry_file):
                with open(registry_file, "r", encoding="utf-8") as f:
                    registry = json.load(f)
                ...
                if current_file in registry:
                    ...
                    if os.path.exists(param_file):
                        with open(param_file, "r", encoding="utf-8") as f:
                            ...
                            st.session_state["param_values"] = param_data.get("param_values", {})
                            
                            # Создаем param_options на основе загруженных данных
                            create_param_options_from_values()
                            return True
            
            # Если параметры для файла не найдены, инициализируем пустые
            st.session_state["param_values"] = {}
...
def analyze_image_with_ai(image_path_or_url: str, api_key: str = None) -> dict:
    """
    ...
    try:
        ...
        # Формируем промпт с учетом существующих параметров
...
- Фокусируйся ТОЛЬКО на характеристиках самого товара: цвет, материал, стиль, длина
- Игнорируй людей на изображении полностью - они просто демонстрируют товар
- Анализируй только визуальные характеристики товара: его цвет, фасон, материал, длину
- Это коммерческий анализ товара для каталога, не анализ людей

Определи следующие параметры ТОВАРА:"""]
        
        # Добавляем информацию о существующих параметрах
        if param_options:
            prompt_parts.append("\nКРИТИЧЕСКИ ВАЖНО: Используй ТОЛЬКО параметры из списка ниже! НЕ добавляй параметры, которых нет в списке!")
            prompt_parts.append("\nСуществующие параметры и их варианты (используй ТОЛЬКО эти параметры):")
            
            # Особые инструкции для параметра "Цвет"
...
КРИТИЧЕСКИ ВАЖНО: 
- ОБЯЗАТЕЛЬНО включи ВСЕ параметры из списка выше в ответ JSON
- Даже если параметр определить сложно, попробуй определить его на основе изображения
- Используй ТОЛЬКО параметры из списка выше
- НЕ добавляй параметры, которых нет в списке (например, не добавляй "Стиль", "Материал", "Длина" если их нет в списке)
- Для параметра "Цвет": если товар имеет ОДИН цвет - используй основной цвет (Белый, Черный, Синий и т.д.)
- Для параметра "Цвет": если товар имеет ДВА разных цвета (например, полоски, комбинация цветов) - используй сочетание (Черно-белый, Сине-белый и т.д.)
- НЕ используй сочетания цветов если товар однотонный!
- Если параметр определить невозможно после тщательного анализа, укажи пустую строку "" (но это должно быть крайне редко - старайся определить все параметры)
- Если есть существующие варианты для параметра, ОБЯЗАТЕЛЬНО выбери один из них
- Если для параметра не указаны варианты (помечено как "определи значение самостоятельно"), определи подходящее значение на основе изображения товара
- Для параметра "Тип": определи тип товара (например: футболка, лонгслив, комплект, худи, свитшот и т.д.)
- Будь внимателен и старайся определить ВСЕ параметры из списка""")
        else:
...
def get_product_params_from_images(url: str, api_key: str = None) -> dict:
    """
    Определяет параметры товара по первым трем фотографиям через нейросеть.
    ...
    try:
        ...
        images = get_product_images(sku, max_images=3)
        ...
        for img_url in images:
            img_params = analyze_image_with_ai(img_url, api_key)
            if img_params:
...
if uploaded is None:
        ...
        if param in display_df.columns:
            if param in st.session_state.get("param_options", {}):
                ...
                if column_data.dtype in ['int64', 'float64'] or pd.api.types.is_numeric_dtype(column_data):
                    ...
                    col_cfg[param] = cc.NumberColumn(param, width=150, format="%.0f")
                else:
    ...
    edited_df = st.data_editor(
                display_df, 
        width='stretch', 
        height=optimal_height,  # Динамическая высота на основе количества строк
                hide_index=True, 
    ...
    with col_save1:
        ...
        with tab2:
            ...
            # ========== РЕДАКТОР ПАРАМЕТРОВ ==========
            with st.expander("📝 Редактор параметров", expanded=True):
                ...
                editor_mode = st.radio(
                    "Режим работы:",
                    ["📋 Просмотр и редактирование", "➕ Добавить новый параметр", "📥 Импорт / 📤 Экспорт"],
                    horizontal=True,
                    key="editor_mode"
                )
                                ...
                                with st.container():
                                    ...
                                    with col_header1:
                                        ...
                                        if st.session_state.get(f"editing_param_name_{param_name}", False):
                                            ...
                                            st.markdown(f"### 📋 {param_name} ({options_count} вариантов)")
            ...
            # Информация о текущем файле и его параметрах
            current_file = st.session_state.get("cached_file_name", "Не выбран")
            st.info(f"📁 **Текущий файл:** {current_file}")
            
            param_count = len(st.session_state.get("param_values", {}))
            st.info(f"📊 **Параметров настроено:** {param_count}")
            
            if param_count > 0:
                param_names = list(st.session_state.get("param_values", {}).keys())
                st.info(f"🏷️ **Параметры:** {', '.join(param_names)}")
            
            # Показываем текущие настройки фильтров
            st.info(f"🔍 **Поиск:** {st.session_state.get('search', '')}")
            st.info(f"💰 **Выручка:** {st.session_state.get('revenue_min', 0):,} - {st.session_state.get('revenue_max', 1000000):,} ₽")
            st.info(f"💵 **Цена:** {st.session_state.get('price_min', 0):,} - {st.session_state.get('price_max', 10000):,} ₽")
            st.info(f"📈 **Процент выкупа:** {st.session_state.get('buyout_pct', 25)}%")
            
            st.divider()
            
            # Кнопки управления параметрами
            col_reload, col_clear_file_params = st.columns(2)
            ...
            with col_clear_file_params:
                if st.button("🗑️ Очистить параметры файла", key="clear_file_params"):
                    st.session_state["param_values"] = {}
                    st.session_state["param_options"] = {}
                    if save_param_values_to_file():
                        st.success("✅ Параметры файла очищены!")
                    else:
                        st.warning("⚠️ Параметры очищены в памяти, но не сохранены в файл")
                    st.rerun()
            ...
            st.write("Вставьте ссылку на товар Wildberries. Система отправит первые 3 фотографии товара в нейросеть для анализа:")
            ...
            with col_url2:
                st.write("")  # Отступ
                st.write("")  # Отступ
                if st.button("🤖 Анализировать через нейросеть", type="primary", key="extract_params_btn"):
                    if product_url:
                        with st.spinner("⏳ Анализ фотографий товара..."):
                            ...
                            if sku:
                                ...
                                # Получаем и анализируем изображения из кеша
                                progress_bar = st.progress(0)
                                status_text = st.empty()
                                ...
                                status_text.text("📥 Поиск изображений в кеше...")
                                progress_bar.progress(20)
                                
                                # Получаем кешированные изображения
                                images = get_cached_images_for_sku(sku, max_images=3)
                                
                                if images:
                                    st.info(f"📸 Найдено изображений в кеше: {len(images)}")
                                    ...
                                    if len(images) > 0:
                                        ...
                                        for idx, img_path in enumerate(images[:3]):
                                            with cols[idx]:
                                                try:
                                                    st.image(img_path, use_container_width=True, caption=f"Фото {idx+1}")
                                                except:
                                    ...
                                    if not OPENAI_AVAILABLE:
                                        ...
                                        if not api_key:
                                            ...
                                            for idx, img_path in enumerate(images[:3]):
                                                status_text.text(f"🤖 Анализ изображения {idx+1}/{len(images)}...")
                                                try:
                                                    ...
                                                    img_params = analyze_image_with_ai(img_path, api_key)
                                                ...
                                                if "Артикул" in display_df.columns:
                                                    ...
                                                    if sku_in_table:
                                                        ...
                                                        for param_name, param_value in final_params.items():
                                                            if param_name not in param_values:
                                                                param_values[param_name] = {}
                                                            param_values[param_name][str(sku)] = str(param_value)
                                    ...
                                    st.warning("⚠️ Изображения товара не найдены в кеше. Убедитесь, что изображения были загружены ранее через основную таблицу.")
                                    st.info("💡 **Совет:** Загрузите файл с данными товаров и включите отображение изображений, чтобы они были скачаны в кеш.")
                                
                                # Показываем ссылку на товар
                                wb_url = build_wb_product_url(sku)
                                st.markdown(f"🔗 [Открыть товар на Wildberries]({wb_url})")
                            else:
                                st.error("❌ Не удалось извлечь артикул из ссылки. Проверьте формат URL.")
                                st.info("""
                                **Поддерживаемые форматы ссылок:**
                                - `https://www.wildberries.ru/catalog/12345678/detail.aspx`
                                - `https://wildberries.ru/catalog/12345678/detail.aspx`
                                - `https://global.wildberries.ru/catalog/12345678/detail.aspx`
                                """)
                    else:
            ...
            # ========== СЕКЦИЯ: Массовый анализ товаров ==========
            st.divider()
            st.subheader("🚀 Массовый анализ товаров через нейросеть")
            st.write("Выберите товары из таблицы для автоматического анализа параметров. Результаты будут автоматически добавлены в таблицу.")
            
            # Проверяем наличие таблицы - используем основной df, так как display_df может быть пустым из-за фильтров
            display_df = st.session_state.get("display_df", pd.DataFrame())
            
            # Проверяем основной датафрейм df, который содержит все данные из файла
            if "Артикул" in df.columns and len(df) > 0:
                # Используем display_df для отображения, но проверяем основной df
                if "Артикул" not in display_df.columns or len(display_df) == 0:
                    # Если display_df пустой из-за фильтров, используем основной df
                    display_df = df.copy()
                # Получаем список всех артикулов
                all_skus = [str(sku).replace(".0", "") for sku in display_df["Артикул"].dropna().unique()]
                all_skus = sorted(list(set(all_skus)))
                
                # Получаем список всех параметров, которые должны быть заполнены
                param_options = st.session_state.get("param_options", {})
                all_param_names = list(param_options.keys()) if param_options else []
                
                # Проверяем, какие артикулы имеют ВСЕ параметры заполненными
                skus_with_all_params = []
                skus_with_some_params = []
                skus_without_params = []
                
                for sku in all_skus:
                    sku_str = str(sku)
                    # Подсчитываем, сколько параметров заполнено для этого товара
                    filled_params_count = 0
                    for param_name in all_param_names:
                        if param_name in param_values and sku_str in param_values[param_name]:
                            param_value = param_values[param_name][sku_str]
                            # Проверяем, что значение не пустое
                            if param_value and str(param_value).strip():
                                filled_params_count += 1
                    
                    if all_param_names:
                        # Если есть параметры для заполнения
                        if filled_params_count == len(all_param_names):
                            # Все параметры заполнены
                            skus_with_all_params.append(sku)
                        elif filled_params_count > 0:
                            # Часть параметров заполнена
                            skus_with_some_params.append(sku)
                        else:
                            # Нет заполненных параметров
                            skus_without_params.append(sku)
                    else:
                        # Если нет параметров для заполнения, считаем что все товары без параметров
                        skus_without_params.append(sku)
                
                # Для обратной совместимости
                skus_with_params_list = skus_with_all_params + skus_with_some_params
                
                if all_param_names:
                    st.info(f"📊 Всего товаров в таблице: **{len(all_skus)}** | "
                           f"С ВСЕМИ параметрами ({len(all_param_names)}): **{len(skus_with_all_params)}** | "
                           f"С частичными параметрами: **{len(skus_with_some_params)}** | "
                           f"Без параметров: **{len(skus_without_params)}**")
                else:
                    st.info(f"📊 Всего товаров в таблице: **{len(all_skus)}** | "
                           f"Параметры не настроены. Добавьте параметры в редакторе.")
                
                # Выбор режима анализа
                analysis_mode = st.radio(
                    "Режим анализа:",
                    ["Анализировать все товары", "Анализировать только товары без параметров", "Выбрать товары вручную"],
                    key="mass_analysis_mode"
                )
                
                selected_skus = []
                
                if analysis_mode == "Анализировать все товары":
                    selected_skus = all_skus
                    st.write(f"✅ Будет проанализировано: **{len(selected_skus)}** товаров")
                
                elif analysis_mode == "Анализировать только товары без параметров":
                    # Включаем товары без параметров и с частичными параметрами
                    selected_skus = skus_without_params + skus_with_some_params
                    if selected_skus:
                        st.write(f"✅ Будет проанализировано: **{len(selected_skus)}** товаров (без параметров: {len(skus_without_params)}, с частичными: {len(skus_with_some_params)})")
                    else:
                        if all_param_names:
                            st.success(f"✅ Все товары имеют все параметры заполненными ({len(all_param_names)} параметров)!")
                        else:
                            st.info("ℹ️ Параметры не настроены. Добавьте параметры в редакторе.")
                
                elif analysis_mode == "Выбрать товары вручную":
                    # Мультиселект для выбора товаров
                    # По умолчанию предлагаем товары без параметров и с частичными параметрами
                    default_skus = (skus_without_params + skus_with_some_params)[:10] if (skus_without_params + skus_with_some_params) else []
                    selected_skus = st.multiselect(
                        "Выберите артикулы для анализа:",
                        options=all_skus,
                        default=default_skus,
                        help="Выберите товары, которые нужно проанализировать. По умолчанию показаны товары без параметров или с частичными параметрами."
                    )
                    if selected_skus:
                        # Показываем статистику по выбранным товарам
                        selected_with_all = len([s for s in selected_skus if s in skus_with_all_params])
                        selected_with_some = len([s for s in selected_skus if s in skus_with_some_params])
                        selected_without = len([s for s in selected_skus if s in skus_without_params])
                        st.write(f"✅ Выбрано товаров: **{len(selected_skus)}** (с всеми параметрами: {selected_with_all}, с частичными: {selected_with_some}, без параметров: {selected_without})")
                    else:
                        st.write(f"✅ Выбрано товаров: **0**")
                
                # Настройки массового анализа
                with st.expander("⚙️ Настройки массового анализа", expanded=False):
                    max_images_per_product = st.slider(
                        "Количество изображений для анализа каждого товара:",
                        min_value=1,
                        max_value=5,
                        value=3,
                        help="Больше изображений = точнее результат, но дольше анализ"
                    )
                    ...
                    delay_between_requests = st.slider(
                        ...
                        value=1.0,
                        step=0.5,
                        help="Помогает избежать лимитов API"
                    )
                
                # Кнопка запуска массового анализа
                if selected_skus:
                    if not OPENAI_AVAILABLE:
                        st.error("⚠️ Для анализа изображений через нейросеть необходимо установить библиотеку: `pip install openai`")
                    else:
                        ...
                        if not api_key:
                            ...
                            if st.button("🚀 Запустить массовый анализ", type="primary", key="start_mass_analysis"):
                                ...
                                with progress_container:
                                    ...
                                    total_products = len(selected_skus)
                                    ...
                                    for idx, sku in enumerate(selected_skus):
                                        main_status.text(f"📦 Анализ товара {idx+1}/{total_products} (Артикул: {sku})...")
                                        main_progress.progress((idx) / total_products)
                                        
                                        try:
                                            # Получаем изображения из кеша
                                            images = get_cached_images_for_sku(sku, max_images=max_images_per_product)
                                            ...
                                            if not images:
                                                ...
                                                results_summary.append({
                                                    "sku": sku,
                                                    "status": "⏭️ Пропущен",
                                                    "reason": "Нет изображений в кеше"
                                                })
                                                continue
                                            
                                            # Определяем комплектность
                                            completeness = determine_completeness(sku)
                                            initial_params = {}
                                            if completeness:
                                                initial_params["Комплект"] = completeness
                                            ...
                                            for img_idx, img_path in enumerate(images[:max_images_per_product]):
                                                try:
                                                    if not os.path.exists(img_path):
                                                        errors_count += 1
                                                        continue
                                                    ...
                                                    errors_count += 1
                                                    if st.session_state.get('debug_ai_analysis', False):
                                                        st.warning(f"⚠️ Ошибка при анализе изображения для {sku}: {str(e)[:100]}")
                                            ...
                                            for key, values in all_params.items():
                                                if values:
                                                    ...
                                                    if most_common and str(most_common).strip():
                                                        final_params[key] = most_common
                                            
                                            # Добавляем комплектность
                                            if initial_params:
                                                final_params.update(initial_params)
                                            
                                            # Проверяем, какие параметры из param_options не были определены
                                            param_options = st.session_state.get("param_options", {})
                                            if param_options:
                                                for param_name in param_options.keys():
                                                    # Если параметр не был определен нейросетью, но есть в param_options
                                                    if param_name not in final_params:
                                                        # Не добавляем пустое значение - оставляем как есть
                                                        # Это позволит пользователю заполнить его вручную позже
                                                        pass
                                            
                                            # Сохраняем параметры (даже если не все определены)
                                            if final_params:
                                                for param_name, param_value in final_params.items():
                                                    if param_name not in param_values:
                                                        param_values[param_name] = {}
                                                ...
                                                results_summary.append({
                                                    "sku": sku,
                                                    "status": "❌ Не определены параметры",
                                                    "reason": "Нейросеть не смогла определить параметры"
                                                })
                                            
                                            # Задержка между запросами
                                            if delay_between_requests > 0 and idx < total_products - 1:
                                                import time
                                    ...
                                    if successful_products > 0:
                                        if save_param_values_to_file():
                                            st.success(f"✅ Параметры сохранены в файл!")
                                    
                                    # Показываем статистику
                                    stats_text.markdown(f"""
                                    ### 📊 Результаты массового анализа:
                                    - ✅ **Успешно проанализировано:** {successful_products} товаров
                                    - ❌ **Ошибки:** {failed_products} товаров
                                    - ⏭️ **Пропущено:** {skipped_products} товаров
                                    - 📦 **Всего обработано:** {total_products} товаров
                                    """)
                                    
                                    # Показываем детальные результаты
                                    with results_container:
                                        with st.expander("📋 Детальные результаты", expanded=False):
                    ...
                    st.info("ℹ️ Выберите товары для анализа")
            else:
                # Проверяем, загружен ли файл вообще
                if uploaded is not None:
                    if "Артикул" not in df.columns:
                        st.warning("⚠️ В загруженном файле отсутствует колонка 'Артикул'. Добавьте колонку с артикулами для использования массового анализа.")
                    elif len(df) == 0:
                        st.warning("⚠️ Загруженный файл не содержит данных. Проверьте файл и попробуйте загрузить снова.")
                    else:
                        st.warning("⚠️ Не удалось определить данные для массового анализа. Попробуйте перезагрузить файл.")
                else:
                    st.warning("⚠️ Загрузите файл с данными товаров, чтобы использовать массовый анализ")
            
            st.divider()
            # ========== КОНЕЦ СЕКЦИИ: Массовый анализ товаров ==========
            ...
            param_templates = {
                ...
                "Цвет": [
                    # Основные цвета (8)
                    "Белый", "Черный", "Серый", "Синий", "Красный", "Зеленый", "Розовый", "Бежевый",
                    # Сочетания двух цветов (использовать ТОЛЬКО когда видно два разных цвета на товаре) (12)
                    "Черно-белый", "Бело-черный",
                    "Сине-белый", "Бело-синий",
                    "Красно-белый", "Бело-красный",
                    "Черно-серый", "Серо-черный",
                    "Сине-черный", "Черно-синий",
                    "Красно-черный", "Черно-красный",
                    "Бело-серый", "Серо-белый",
                    "Сине-серый", "Серо-синий",
                    "Бело-зеленый", "Зелено-белый",
                    "Бело-розовый", "Розово-белый",
                    "Черно-зеленый", "Зелено-черный"
                ],
                "Длина": ["Короткая", "Средняя", "Длинная", "Мини", "Макси", "Миди", "Анкл"],
                "Пуговицы": ["С пуговицами", "Без пуговиц", "На молнии", "На липучке", "На кнопках", "На шнурке"],
                "Материал": ["Хлопок", "Полиэстер", "Шерсть", "Лен", "Джинс", "Кожа", "Замша", "Трикотаж"],
            ...
            with col_add:
                # Проверяем, существует ли уже параметр с таким именем
                param_exists = param_name and param_name in st.session_state.get("param_options", {})
                
                if param_exists:
                    button_text = "🔄 Обновить параметр"
                    button_type = "secondary"
                else:
                    button_text = "➕ Добавить параметр"
                    button_type = "primary"
                
                if st.button(button_text, type=button_type):
                    if param_name:
                        try:
                            # Очищаем и разбираем варианты (если указаны)
                            if param_options:
                                options = [opt.strip() for opt in param_options.split("/") if opt.strip()]
                            else:
                                options = []  # Пустой список, если варианты не указаны
                            
                            # Создаем или обновляем параметр (даже если вариантов нет)
                            if param_name not in param_values:
                                param_values[param_name] = {}
                            ...
                            old_count = len(st.session_state["param_options"].get(param_name, []))
                            st.session_state["param_options"][param_name] = options
                            new_count = len(options)
                            
                            if param_exists:
                                if new_count > 0:
                                    st.success(f"✅ Параметр '{param_name}' обновлен: было {old_count} вариантов, стало {new_count} вариантов")
                                else:
                                    st.success(f"✅ Параметр '{param_name}' обновлен (без ограничений по вариантам)")
                            else:
                                if new_count > 0:
                                    st.success(f"✅ Параметр '{param_name}' добавлен с вариантами: {', '.join(options[:5])}{'...' if len(options) > 5 else ''}")
                                else:
                                    st.success(f"✅ Параметр '{param_name}' добавлен (без ограничений по вариантам - нейросеть определит значение самостоятельно)")
                            ...
                            if "temp_param_options" in st.session_state:
                                ...
                                if save_param_values_to_file():
                                    if param_exists:
                                        st.success(f"✅ Параметр '{param_name}' обновлен и сохранен в файл!")
                                    else:
                                        st.success(f"✅ Параметр '{param_name}' добавлен и сохранен в файл!")
                                else:
                                    st.warning("⚠️ Параметр добавлен, но не сохранен в файл")
                            else:
            ...
            with col_clear:
                if st.button("🗑️ Очистить все"):
                    ...
                    if "temp_param_options" in st.session_state:
                        del st.session_state["temp_param_options"]
                    if save_param_values_to_file():
                        st.success("✅ Все параметры удалены и изменения сохранены в файл!")
                    else:
                        st.warning("⚠️ Параметры удалены, но изменения не сохранены в файл")
                ...
                for col_idx, (param_name, options) in enumerate(row_params):
                    with param_cols[col_idx]:
                        with st.expander(f"📋 {param_name} ({len(options)} вариантов)"):
                            # Кнопка для обновления параметра из шаблона (если есть шаблон)
                            if param_name in param_templates:
                                template_options = param_templates[param_name]
                                if len(template_options) > len(options):
                                    if st.button(f"🔄 Обновить из шаблона ({len(template_options)} вариантов)", key=f"update_from_template_{param_name}"):
                                        # Обновляем варианты из шаблона
                                        st.session_state["param_options"][param_name] = template_options.copy()
                                        if save_param_values_to_file():
                                            st.success(f"✅ Параметр '{param_name}' обновлен из шаблона! Теперь {len(template_options)} вариантов")
                                            st.rerun()
                                        else:
                                            st.warning("⚠️ Параметр обновлен, но не сохранен в файл")
            ...
            if "Артикул" in display_df.columns:
                ...
                # Автоматически сохраняем изменения при редактировании таблицы
                changes_made = False
                for index, row in edited_df.iterrows():
                    sku = row["Артикул"]
                    for param in all_params:
