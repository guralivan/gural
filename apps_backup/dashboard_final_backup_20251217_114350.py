...
# Временно отключен кеш для отладки
# @st.cache_data(show_spinner=False, ttl=3600, hash_funcs={tuple: lambda x: hash(x)})
def find_and_load_reports_from_tovar(skus: tuple, tovar_folder: str = "Tovar") -> dict:
    """
...
if uploaded is None:
    ...
    if df is None or df.empty:
        ...
        if seasonality_available:
            if PROPHET_AVAILABLE:
                tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["📊 Анализ данных", "⚙️ Установка параметров", "📈 Аналитика по параметрам", "📅 Анализ сезонности", "🔮 Прогнозирование", "🗺️ План продаж", "📦 Расчет заказа", "📜 История изменений"])
            else:
                tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Анализ данных", "⚙️ Установка параметров", "📈 Аналитика по параметрам", "📅 Анализ сезонности", "🗺️ План продаж", "📦 Расчет заказа", "📜 История изменений"])
        else:
            if PROPHET_AVAILABLE:
                tab1, tab2, tab3, tab4, tab5, tab6, tab7 = st.tabs(["📊 Анализ данных", "⚙️ Установка параметров", "📈 Аналитика по параметрам", "🔮 Прогнозирование", "🗺️ План продаж", "📦 Расчет заказа", "📜 История изменений"])
            else:
                tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["📊 Анализ данных", "⚙️ Установка параметров", "📈 Аналитика по параметрам", "🗺️ План продаж", "📦 Расчет заказа", "📜 История изменений"])
    ...
    with col_save1:
        ...
        if seasonality_available:
            if PROPHET_AVAILABLE:
                sales_plan_tab = tab6  # После tab5 (Прогнозирование)
            else:
                sales_plan_tab = tab5  # После tab4 (Анализ сезонности)
        else:
            if PROPHET_AVAILABLE:
                                        ...
                                        if combined_df.empty:
                                            ...
                                            if len(daily_plan_details) > 0:
                                                ...
                                                if show_table_with_trend:
                                                    # Пытаемся получить combined_plan_details из locals или session_state
                                                    if 'combined_plan_details' in locals() and len(combined_plan_details) > 0:
                                                        table_source = combined_plan_details
                                                    else:
                                                        # Пытаемся получить из session_state
                                                ...
                                                # Сохраняем объединенную таблицу в session_state для использования во вкладке "Расчет заказа"
                                                st.session_state[f'plan_details_table_{selected_combo_key}'] = combined_plan_details
                                            else:
                                                # Если нет прогноза, используем только daily_plan_details
                                                combined_plan_details = daily_plan_details.copy()
        ...
        # - tab4: Анализ сезонности (если seasonality_available)
        ...
        if seasonality_available:
            if PROPHET_AVAILABLE:
                order_calc_tab = tab7  # tab7 = "📦 Расчет заказа"
            else:
                            ...
                            if plan_details_table is None:
                                ...
                                with col4:
                                    ...
                                    safety_stock_pct = st.number_input(
                                        ...
                                        min_value=0,
                                        max_value=100,
                                        value=st.session_state.get(f"safety_stock_pct_{plan_key}", 20),
                                        step=1,
