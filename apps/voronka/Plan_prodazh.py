...
def calculate_complex_rentability(average_price, cost_price, discount_percent=0, commission_rate=15, 
    ...
    # 4. Доставка (сложная формула с учетом процента выкупа)
    buyout_ratio = buyout_percent / 100
    delivery_cost = (buyout_ratio * logistics_cost + (1 - buyout_ratio) * (logistics_cost + 50)) * 100 / buyout_percent
...
if 'rentability_params' not in st.session_state:
    st.session_state.rentability_params = {
        ...
        'buyout_percent': 85.0,  # % выкупа
        'storage_cost': 0.0,  # Хранение
        'spp_discount': 25.0  # СПП скидка
    }
else:
    st.info("📋 Настройки рентабельности загружены из кеша")
...
# % выкупа
    ...
    min_value=0.0,
    max_value=100.0,
    value=st.session_state.rentability_params.get('buyout_percent', 85.0),
    step=1.0,
...
if advertising_percent != st.session_state.rentability_params.get('advertising_percent', 0.0):
    ...
    params_changed = True
if buyout_percent != st.session_state.rentability_params.get('buyout_percent', 85.0):
...
if params_changed:
    ...
    st.session_state.rentability_cache = {}
    st.success("✅ Настройки рентабельности сохранены и кешированы!")
    ...
    if orders_col and sales_col:
        ...
        for idx_name in index_names:
            if idx_name == "Средняя цена":
                ...
                values = []
                buyout_percent = st.session_state.rentability_params.get('buyout_percent', 85.0)
...
if df is not None:
    ...
    with st.expander("➕ Добавить товар для расчета рентабельности", expanded=False):
        ...
        with col3:
            ...
            buyout_percent = st.number_input("Выкуп, %", min_value=0.0, max_value=100.0, value=85.0, step=1.0, key="new_buyout_percent")
