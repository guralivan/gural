...
# Сравнение по периодам (месяцы, конкретные даты, дни недели)
st.header("📅 Сравнение по периодам")

# Выбор типа сравнения
comparison_type = st.selectbox(
    "Выберите тип сравнения",
    ["По месяцам", "По конкретным датам", "По дням недели"],
...
elif comparison_type == "По дням недели":
    # Получаем доступные дни недели
    available_days = filtered_df['День_недели'].unique()
    available_years = filtered_df['Год'].unique()
    
    col1, col2 = st.columns(2)
    
    with col1:
        selected_day = st.selectbox("Выберите день недели для сравнения", available_days)
        
    with col2:
        st.write("**Сравнение по годам:**")
        for year in sorted(available_years):
            day_data = filtered_df[
                (filtered_df['День_недели'] == selected_day) & 
                (filtered_df['Год'] == year)
            ]
            if not day_data.empty:
                orders = day_data['Заказали, шт'].sum()
                sales = day_data['Выкупили, шт'].sum()
                revenue = day_data['Выкупили на сумму, ₽'].sum()
                st.write(f"**{year} ({selected_day}):** Заказы: {orders:,.0f}, Выкупы: {sales:,.0f}, Выручка: {revenue:,.0f} ₽")
    
    # График сравнения по дням недели
    if st.button("Показать график сравнения по дням недели"):
        day_comparison = filtered_df[filtered_df['День_недели'] == selected_day].groupby('Год').agg({
            'Заказали, шт': 'sum',
            'Выкупили, шт': 'sum',
            'Выкупили на сумму, ₽': 'sum'
        }).reset_index()
        ...
        fig = make_subplots(
            rows=1, cols=3,
            subplot_titles=(f'Заказы в {selected_day}', f'Выкупы в {selected_day}', f'Выручка в {selected_day}'),
            specs=[[{"type": "bar"}, {"type": "bar"}, {"type": "bar"}]]
        )
        
        fig.add_trace(
            go.Bar(x=day_comparison['Год'], y=day_comparison['Заказали, шт'], 
                   name='Заказы', marker_color=['blue', 'red']),
            row=1, col=1
        )
        
        fig.add_trace(
            go.Bar(x=day_comparison['Год'], y=day_comparison['Выкупили, шт'], 
                   name='Выкупы', marker_color=['green', 'orange']),
            row=1, col=2
        )
        
        fig.add_trace(
            go.Bar(x=day_comparison['Год'], y=day_comparison['Выкупили на сумму, ₽'], 
                   name='Выручка', marker_color=['purple', 'brown']),
            row=1, col=3
        )
        
        fig.update_layout(height=400, title_text=f"Сравнение {selected_day} по годам")
        st.plotly_chart(fig, width='stretch')
